...
if [ "$DEPLOYMENT_GROUP_NAME" = "pre_production" ]; then
    ...
    sed -i'' "s%files.url=.*%files.url=https://files.facilio.in%g" $CONF_DIR/awsprops.properties
    sed -i'' "s%portal.domain=.*%portal.domain=facilstack.com%g" $CONF_DIR/awsprops.properties
    sed -i'' "s%cors.allowed.origins=.*%cors.allowed.origins=http://localhost:8080,http://localhost:9090,https://facilio.in,https://facilstack.com%g" $CONF_DIR/awsprops.properties
...
if [ "$DEPLOYMENT_GROUP_NAME" = "production_deployment" ]; then
    ...
    sed -i'' "s%files.url=.*%files.url=https://files.facilio.com%g" $CONF_DIR/awsprops.properties
