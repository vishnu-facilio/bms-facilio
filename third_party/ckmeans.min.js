/** ckmeans 1.0.1
 * From https://unpkg.com/ckmeans@1.0.1
 */
...
function ssq(j, i, sumX, sumXsq) {
  let sji // s(j, i)
  if (j > 0) {
    let muji = (sumX[i] - sumX[j - 1]) / (i - j + 1) // mu(j, i)
...
function fillMatrixColumn(
  ...
  let jlow = column // the lower end for j
...
function fillMatrices(data, matrix, backtrackMatrix) {
  ...
  let sumX = new Array(nValues)
  let sumXsq = new Array(nValues)
...
function ckmeans(data, nClusters) {
  ...
  let nValues = data.length

  let sorted = numericSort(data)
  // we'll use this as the maximum number of clusters
  let uniqueCount = uniqueCountSorted(sorted)
  ...
  // named 'S' originally
  let matrix = makeMatrix(nClusters, nValues)
  // named 'J' originally
  let backtrackMatrix = makeMatrix(nClusters, nValues)
  ...
  for (let cluster = backtrackMatrix.length - 1; cluster >= 0; cluster--) {
    let clusterLeft = backtrackMatrix[cluster][clusterRight]
