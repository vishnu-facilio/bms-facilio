...
CREATE TABLE IF NOT EXISTS Assets (
	...
	CONSTRAINT ASSET_FK_CATEGORY FOREIGN KEY (ASSET_TYPE) REFERENCES Asset_Categories(ID),
...
CREATE TABLE IF NOT EXISTS Energy_Meter (
	...
	CONSTRAINT ENERGY_METER_FK_PURPOSE_SPACE_ID FOREIGN KEY (PURPOSE_SPACE_ID) REFERENCES Energy_Meter_Purpose(ID)
);
...
CREATE TABLE IF NOT EXISTS Controller (
	...
	JOB_STATUS BOOLEAN,
	STATUS INT,
	CONSTRAINT CONTROLLER_FK_ASSET_ID FOREIGN KEY (CONTROLLER_ID) REFERENCES Assets(ID),
...
CREATE TABLE IF NOT EXISTS Device (
	...
	STATUS INT,
	CONSTRAINT DEVICE_FK_DEVICE_ID FOREIGN KEY (DEVICE_ID) REFERENCES Assets(ID),
...
-- Create indexes for Energy_Data table
CREATE INDEX ENERGY_DATA_TIME_INDEX on Energy_Data (TTIME);
CREATE INDEX ENERGY_DATA_DATE_INDEX on Energy_Data (TTIME_DATE);
...
CREATE TABLE IF NOT EXISTS Alarm_Template (
  	...
  	CONSTRAINT ALARM_TEMPLATE_ID_FK FOREIGN KEY (ID) REFERENCES Templates(ID) ON DELETE CASCADE
);
...
SELECT COALESCE(MAX(LOCAL_ID),0) INTO MAX_LOCAL_ID FROM BaseSpace WHERE ORGID = NEW.ORGID;
