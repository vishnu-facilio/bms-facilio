...
CREATE TABLE PM_Reminders_WO_Rel (
	PM_REMINDER_JOBID BIGINT,
	WO_ID BIGINT,
	PRIMARY KEY (PM_REMINDER_JOBID, WO_ID),
	CONSTRAINT PM_REMINDERS_WO_REL_FK_JOBID FOREIGN KEY (PM_REMINDER_JOBID) REFERENCES Jobs(JOBID) ON DELETE CASCADE,
	CONSTRAINT PM_REMINDERS_WO_REL_FK_WO_ID FOREIGN KEY (WO_ID) REFERENCES WorkOrders(ID) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS Widget_Period (
  ...
  DATE_OPERATOR INT NOT NULL,
  TIME_SERIES_FIELD BIGINT(20) DEFAULT NULL,
  CONSTRAINT WIDGET_PERIOD_WIDGET_ID FOREIGN KEY (WIDGET_ID) REFERENCES Widget (ID),
  CONSTRAINT WIDGET_PERIOD_TIME_SERIES_FIELD FOREIGN KEY (TIME_SERIES_FIELD) REFERENCES Fields (FIELDID)
);
...
CREATE TABLE IF NOT EXISTS Widget_Chart (
  ...
  CHART_DISPLAY_TYPE INT DEFAULT NULL,
  ...
  CONSTRAINT WIDGET_CHART_ID FOREIGN KEY (ID) REFERENCES Widget (ID)
);
...
CREATE TABLE IF NOT EXISTS Widget_List_View (
  ...
  HEADER_TEXT varchar(45) DEFAULT NULL,
  HEADER_SUB_TEXT varchar(45) DEFAULT NULL,
  HEADER_IS_EXPORT tinyint(4) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS Widget_Condition (
  ID BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  WIDGET_CHART_ID BIGINT DEFAULT NULL,
  CRITERIA_ID BIGINT DEFAULT NULL,
  CONSTRAINT WIDGET_COMPARISION_CONDITIONS_CRITETIA_ID FOREIGN KEY (CRITERIA_ID) REFERENCES Criteria (CRITERIAID),
  CONSTRAINT WIDGET_COMPARISION_CONDITIONS_WIDGET_ID FOREIGN KEY (WIDGET_CHART_ID) REFERENCES Widget_Chart (ID)
);
...
CREATE TABLE IF NOT EXISTS View_Column (
	...
	CONSTRAINT VIEW_COLUMN_VIEWID_FK FOREIGN KEY (VIEWID) REFERENCES Views(ID),
	CONSTRAINT VIEW_COLUMN_FIELDID_FK FOREIGN KEY (FIELDID) REFERENCES Fields(FIELDID)
);
