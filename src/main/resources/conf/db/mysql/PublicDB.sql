...
CREATE TABLE IF NOT EXISTS Account_Users (
	...
	ZIP VARCHAR(255),
	COUNTRY VARCHAR(255),
	IDENTIFIER INTEGER DEFAULT 1,
	DOMAIN_NAME VARCHAR(200),
	APP_TYPE TINYINT,
	UNIQUE(EMAIL,PASSWORD,IDENTIFIER)
);
...
CREATE TABLE IF NOT EXISTS FacilioInstantJobQueue_Data(
  QUEUE_ID BIGINT PRIMARY KEY,
  DATA MEDIUMTEXT
);

CREATE TABLE IF NOT EXISTS FacilioExceptionQueue_Data(
  QUEUE_ID BIGINT PRIMARY KEY,
  DATA MEDIUMTEXT
);
...
CREATE TABLE IF NOT EXISTS FacilioRuleInstantJobQueue_Data(
  QUEUE_ID BIGINT PRIMARY KEY,
  DATA MEDIUMTEXT
);
...
CREATE TABLE IF NOT EXISTS Secret_File (
 ...
 CREATE TABLE IF NOT EXISTS Account_User_Apps (
    ID BIGINT AUTO_INCREMENT PRIMARY KEY,
    USERID BIGINT NOT NULL,
    APP_DOMAIN_ID BIGINT NOT NULL,
    CONSTRAINT FK_Account_User_Apps_UserId FOREIGN KEY (USERID) REFERENCES Account_Users(USERID),
    CONSTRAINT FK_Account_User_Apps_DomainId FOREIGN KEY (APP_DOMAIN_ID) REFERENCES App_Domain(ID)
   );
   
  CREATE TABLE IF NOT EXISTS Account_User_Apps_Orgs (
    ID BIGINT AUTO_INCREMENT PRIMARY KEY,
    ACCOUNT_USER_APPID BIGINT,
    ORGID BIGINT NOT NULL,
    ISDEFAULT BOOLEAN,
	USER_STATUS BOOLEAN,
	DELETED_TIME BIGINT DEFAULT -1,
	UNIQUE(ACCOUNT_USER_APPID,ORGID,DELETED_TIME),
	CONSTRAINT FK_Account_User_Apps_Orgs_account_user_app FOREIGN KEY (ACCOUNT_USER_APPID) REFERENCES Account_User_Apps(ID),
    CONSTRAINT FK_Account_User_Apps_Orgs_orgid FOREIGN KEY (ORGID) REFERENCES Organizations(ORGID)
   );
...
INSERT INTO App_Domain (DOMAIN, APP_DOMAIN_TYPE, APP_GROUP_TYPE) VALUES ('localhost', 1, 1);
