...
public class NotesUtil {
    public static List<NoteContext> getNotesFromFile(InputStream inputStream, FacilioModule notesModule, Map<ComponentType, List<PackageChangeSetMappingContext>> packageChangSets, FacilioModule parentModule, DataMigrationBean targetConnection, DataMigrationStatusContext dataMigrationObj) throws Exception {

        List<NoteContext> allNotes = new ArrayList<>();
        List<NoteContext> notes = new ArrayList<>();
        if (inputStream == null || notesModule == null) {
            return allNotes;
        }
        ...
        try (CSVReader csvReader = new CSVReader(new InputStreamReader(inputStream, StandardCharsets.UTF_8))) {
            ...
            for (int i = 1; i < csvData.size(); i++) {
                ...
                NoteContext noteContext = getCsvNote(fieldNames, fieldValues, notesModule, packageChangSets, parentModule, moduleIdVsOldIds);
    ...
    private static NoteContext getCsvNote(String[] fieldNames, String[] fieldValues, FacilioModule noteModule, Map<ComponentType, List<PackageChangeSetMappingContext>> packageChangSets, FacilioModule parentModule, Map<Long, List<Long>> moduleIdVsOldIds) throws Exception {
                    ...
                    if (CollectionUtils.isNotEmpty(commentAttachments)) {
                        ...
                        for (CommentAttachmentContext commentAttachment : commentAttachments) {
                            ...
                            if (Objects.equals(fieldName, "attachmentFileName")) {
                                ...
                                String filePath = DataPackageFileUtil.getAttachmentFilePath(value);
