...
import static com.facilio.agentnew.controller.NewControllerAPI.getControllerFromDb;

public class NewControllerUtil
{

    private static final Logger LOGGER = LogManager.getLogger(NewControllerUtil.class.getName());
    private long ogdId;
    private long agentId;
    List<Map<String, Controller>> controllerMapList = new ArrayList<>();


    public NewControllerUtil(long agentId) {
    ...
    public Controller getController(long agentId, String controllerIdentifier, FacilioControllerType controllerType){
        ...
        if(controllerType == null){
            return NewControllerAPI.getAllControllersFromDb(agentId).get(controllerIdentifier);
        }
        else if( (controllerMapList.get(controllerType.asInt()).isEmpty()) ){
            Map<String, Controller> controllers = NewControllerAPI.getControllersFromDb(agentId,controllerType);
                ...
                if ((controllerProps != null) && (!controllerProps.isEmpty())) {
                    ...
                    if (controller != null) {
                        ...
                        if (controllerFromDb != null) {
                            ...
                            long controllerId = NewControllerAPI.addController(controller);
...
public static Controller makeCustomController(long orgId, long agentId, String name ){
    ...
    controller = (CustomController) NewControllerAPI.getControllerFromDb(name,agentId,FacilioControllerType.CUSTOM);
    if(controller == null){
        ...
        if(NewControllerAPI.addController(controller) < 1){
            controller = null;
        }
    }
        return controller;
 ...
 public JSONArray getAllControllerList(){
     ...
     for (FacilioControllerType type : FacilioControllerType.values()) {
         controllers.putAll(NewControllerAPI.getControllersFromDb(agentId, type));
     }
