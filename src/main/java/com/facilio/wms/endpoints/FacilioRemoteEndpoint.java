 ...
 */
@ServerEndpoint(
		value="/websocket/remote/{id}",
        decoders = MessageDecoder.class,
        encoders = MessageEncoder.class
)
public class FacilioRemoteEndpoint 
{
    private final Logger log = Logger.getLogger(getClass().getName());

    private Session session;
    private static final Set<FacilioRemoteEndpoint> endpoints = new CopyOnWriteArraySet<FacilioRemoteEndpoint>();
...
//    	System.out.println("Session started uid ::" +uid + ":::sessionid" + session.getId());
        ...
        SessionManager.RemoteSession rs = new SessionManager.RemoteSession().setId(id).setSession(session).setCreatedTime(System.currentTimeMillis());
        SessionManager.getInstance().addRemoteSession(rs);
    }
...
//    	System.out.println("Session started closed" +session.getId());
    	...
    	SessionManager.getInstance().removeRemoteSession(session.getId());
    	session.close();
    }
...
//        throwable.printStackTrace();
    ...
    {
        for (FacilioRemoteEndpoint endpoint : endpoints) 
        {
