...
public class ScriptBeanCacheImpl extends ScriptBeanImpl implements ScriptBean {
	...
	public WorkflowNamespaceContext addNameSpace(WorkflowNamespaceContext nameSpace) throws Exception {
        ...
        if (nameSpace != null) {
             FacilioCache<String, WorkflowNamespaceContext> nameSpaceCache = FWLRUScriptCache.getScriptNameSpaceCache();
            String key = FWLRUScriptCache.CacheKeys.SCRIPT_NAMESPACE_KEY(getOrgId(), nameSpace.getName());
	...
	public WorkflowNamespaceContext getNameSpace(String nameSpaceName) throws Exception {
		// TODO Auto-generated method stub
		FacilioCache<String, WorkflowNamespaceContext> nameSpaceCache = FWLRUScriptCache.getScriptNameSpaceCache();
        String key = FWLRUScriptCache.CacheKeys.SCRIPT_NAMESPACE_KEY(getOrgId(), nameSpaceName);
	...
	public FacilioContext addFunction(WorkflowUserFunctionContext function) throws Exception {
        ...
        if (function != null) {
             FacilioCache<String, WorkflowUserFunctionContext> functionCache = FWLRUScriptCache.getScriptFunctionCache();
            String key = FWLRUScriptCache.CacheKeys.SCRIPT_FUNCTION_KEY(getOrgId(), function.getNameSpaceName(), function.getName());
	...
	public FacilioContext updateFunction(WorkflowUserFunctionContext function) throws Exception {
        ...
        if (olduserFunction != null) {
             FacilioCache<String, WorkflowUserFunctionContext> functionCache = FWLRUScriptCache.getScriptFunctionCache();
            String key = FWLRUScriptCache.CacheKeys.SCRIPT_FUNCTION_KEY(getOrgId(), olduserFunction.getNameSpaceName(), olduserFunction.getName());
            functionCache.remove(key);
        }
	...
	public WorkflowUserFunctionContext deleteFunction(WorkflowUserFunctionContext function) throws Exception {
		...
		if (userfunction != null) {
            FacilioCache<String, WorkflowUserFunctionContext> functionCache = FWLRUScriptCache.getScriptFunctionCache();
           String key = FWLRUScriptCache.CacheKeys.SCRIPT_FUNCTION_KEY(getOrgId(), userfunction.getNameSpaceName(), userfunction.getName());
           functionCache.remove(key);
        }
	...
	public WorkflowUserFunctionContext getFunction(String nameSpaceName, String functionName) throws Exception {
		// TODO Auto-generated method stub
		FacilioCache<String, WorkflowUserFunctionContext> functionCache = FWLRUScriptCache.getScriptFunctionCache();
        String key = FWLRUScriptCache.CacheKeys.SCRIPT_FUNCTION_KEY(getOrgId(), nameSpaceName, functionName);
