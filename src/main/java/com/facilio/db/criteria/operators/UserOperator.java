...
@Log4j
public enum UserOperator implements Operator<Criteria> {

    USER(127, "user");

    private UserOperator(int operatorId, String operator) {
        ...
        try {
            if(fieldName != null && !fieldName.isEmpty() && value != null) {
                ...
                if(module.length > 1) {
                    ...
                    FacilioModule lookupModule = modBean.getModule(FacilioConstants.ContextNames.USERS);
                    if(module != null) {
                        return LookupCriteriaUtil.constructLookupQuery(lookupField,FieldFactory.getAsMap(FieldFactory.getOrgUserFields()).get("orgUserId"),lookupModule,value);
                    }
                }
            }
        } catch (Exception e) {
