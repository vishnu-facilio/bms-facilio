...
//		Boolean isHistorical = (Boolean) conWorkflowContext.java:458text.get(FacilioConstants.ContextNames.IS_HISTORICAL);
...
//	}
    ...
    public boolean isMatchingResource(ReadingContext reading) {
            ...
            if (workflowRule.getRuleTypeEnum().isChildSupport() && !workflowRule.shouldTerminateChildExecution()) {
                ...
                if (currentWorkflows == null) {
                    ...
                    if (currentWorkflows != null) {
                        ...
                        LOGGER.debug("Current workflows is null, module : " + module + " , event type : " + eventTypes + " , criteria : " + criteria + ", rule types : " + ruleTypes);
...
//		return false;
    ...
    public boolean executeRuleAndChildren(Object record, List<UpdateChangeSet> changeSet, Map<String, Object> recordPlaceHolders, FacilioContext context, boolean propagateError, boolean isParallelRuleExecution, RuleType... ruleTypes) throws Exception {

        List<EventType> eventTypes = Collections.singletonList(this.getActivityTypeEnum());

        Map<String, List<WorkflowRuleContext>> workflowRuleCacheMap = new HashMap<>();
        ...
        FacilioModule module = this.getReadingField().getModule();
        WorkflowRuleContext workflowRule = this;
        ...
        try {
            ...
            if (workflowRule.getRuleTypeEnum().isChildSupport() && !workflowRule.shouldTerminateChildExecution()) {
                ...
                if (currentWorkflows == null) {
                    Criteria criteria = new Criteria();
                    criteria.addAndCondition(CriteriaAPI.getCondition(parentRule, String.valueOf(workflowRule.getId()), NumberOperators.EQUALS));
                    criteria.addAndCondition(CriteriaAPI.getCondition(onSuccess, String.valueOf(result), BooleanOperators.IS));
                    currentWorkflows = WorkflowRuleAPI.getActiveWorkflowRulesFromActivityAndRuleType(workflowRule.getModule(), eventTypes, criteria, false, true, ruleTypes);
