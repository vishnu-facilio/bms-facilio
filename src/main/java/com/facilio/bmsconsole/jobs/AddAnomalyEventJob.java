...
//          long checkratioModuleid2 = 77887; // IF there is more than one set in ratio hierarchy [1,2,3,4] , [4,5,6,7]
			...
			List<Map<String, Object>> props1 = getData(checkratio1Fields, checkRatio1Module, startTime,
					startTime + 360000, checkGamParentList[0]);
			for (Map<String, Object> prop : props1) {
			...
			if (ratio1data.length() > 0) {
				...
				if (actualValue > adjustedUpperBound) { // ML ID
														// select ML_ID from ML_Variables M inner JOIN Modules ms ON
														// M.MODULEID=ms.MODULEID WHERE ms.NAME='energydata' AND
														// IS_SOURCE=1 AND M.ORGID=? and M.PARENT_ID=? order by ID desc
														// LIMIT 1
					MLAnomalyEvent parentEvent = generateMLAnomalyEvent(Long.parseLong(checkGamParentList[0]),
							actualValue, adjustedUpperBound, startTime, mlList[0]);
					...
					for (int i = 1; i < mlList.length; i++) {
						if (ratio1data.has(checkGamParentList[i] + "_ratio")) {
							...
							MLAnomalyEvent event = generateRCAEvent(Long.parseLong(checkGamParentList[i]),
									(Double) data.get("actualValue"), (Double) data.get("adjustedUpperBound"),
									startTime, mlList[i], (Double) ratio1data.get(checkGamParentList[i] + "_ratio"),
									parentEvent);
							eventList.add(event);
						}
					}
				...
				MLAnomalyEvent event = checkAndGenerateMLAnomalyEvent(Long.parseLong(checkGamParentList[0]),
						checkGamData.get(checkGamParentList[0]), startTime, mlList[0]);
				if (event != null) {
					eventList.add(event);
				}
				for (int i = 1; i < mlList.length; i++) {
