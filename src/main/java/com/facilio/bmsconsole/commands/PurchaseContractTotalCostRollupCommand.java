...
public class PurchaseContractTotalCostRollupCommand extends FacilioCommand {
	
	@Override
	public boolean executeCommand(Context context) throws Exception {
		PurchaseContractContext purchaseContract = (PurchaseContractContext) context.get(FacilioConstants.ContextNames.RECORD);
		if(purchaseContract!=null) {
			ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
			FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.PURCHASE_CONTRACTS);
			double totalCost = getTotalCost(purchaseContract.getId());
			purchaseContract.setTotalCost(totalCost);
			...
			UpdateRecordBuilder<PurchaseContractContext> updateBuilder = new UpdateRecordBuilder<PurchaseContractContext>()
					...
					.andCondition(CriteriaAPI.getIdCondition(purchaseContract.getId(), module));
			updateBuilder.update(purchaseContract);
		}
		return false;
	}
	...
	private double getTotalCost(long id) throws Exception {
		...
		List<FacilioField> linefields = modBean.getAllFields(FacilioConstants.ContextNames.PURCHASE_CONTRACTS_LINE_ITEMS);
		...
		SelectRecordsBuilder<PurchaseContractLineItemContext> builder = new SelectRecordsBuilder<PurchaseContractLineItemContext>()
				...
				.andCondition(CriteriaAPI.getCondition(fieldsMap.get("purchaseContractId"), String.valueOf(id), NumberOperators.EQUALS))
				.setAggregation()
				;
