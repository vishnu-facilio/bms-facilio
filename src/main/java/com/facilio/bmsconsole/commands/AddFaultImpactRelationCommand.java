            ...
            if (CollectionUtils.isNotEmpty(list)) {
                for (NewReadingRuleContext readingRule : list) {
                    if (readingRule.getImpact() != null && readingRule.getImpact().getId() != -1) {
                        ...
                        //TODO:make generic once execmode changes in fault impact
                        NameSpaceContext impactNameSpaceContext = new NameSpaceContext(NSType.FAULT_IMPACT_RULE, readingRule.getId(), readingRule.getNs().getExecInterval(), readingRule.getImpact().getWorkflowId(), categoryId, NSExecMode.WORKFLOW.getValue());
