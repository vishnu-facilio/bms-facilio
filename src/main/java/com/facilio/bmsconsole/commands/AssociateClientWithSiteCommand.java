...
public class AssociateClientWithSiteCommand extends FacilioCommand {

	@Override
	public boolean executeCommand(Context context) throws Exception {
		List<Long> siteIds = (List<Long>) context.get(FacilioConstants.ContextNames.SITE_LIST);
		long clientId = (long) context.get(FacilioConstants.ContextNames.RECORD_ID);
		if (siteIds != null && !siteIds.isEmpty() && clientId > 0) {
			...
			UpdateRecordBuilder<InventoryRequestLineItemContext> updateBuilder = new UpdateRecordBuilder<InventoryRequestLineItemContext>()
					.module(module).fields(fields).andCondition(CriteriaAPI.getIdCondition(siteIds, module));
