...
public class GenerateGenericSearchConditionCommand extends FacilioCommand {
	private static final Logger LOGGER = LogManager.getLogger(GenerateGenericSearchConditionCommand.class.getName());
	@Override
	public boolean executeCommand(Context context) throws Exception {
		...
		Object search = context.get(FacilioConstants.ContextNames.GENERIC_SEARCH);
		if (search instanceof JSONObject) {
			JSONObject searchObj = (JSONObject) context.get(FacilioConstants.ContextNames.GENERIC_SEARCH);
			if (searchObj != null && !searchObj.isEmpty()) {
				...
				context.put(FacilioConstants.ContextNames.GENERIC_SEARCH_CRITERIA, criteria);
			}
		}
			...
			context.put(FacilioConstants.ContextNames.GENERIC_SEARCH_CRITERIA, constructMainFieldSearchCriteria(primaryField, (String) search, modBean));
		}
		long timeTaken = System.currentTimeMillis() - startTime;
		LOGGER.debug("Time taken to execute GenerateGenericSearchConditionCommand : "+timeTaken);
		return false;
	}
