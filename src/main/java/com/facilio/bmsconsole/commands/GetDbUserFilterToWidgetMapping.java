			...
			if (widgets != null && userFilters != null) {
				...
				for (DashboardWidgetContext widget : widgets) {
					...
					for (DashboardUserFilterContext filter : userFilters) {
						...
						// for all  other widget , map only the filters that apply to them
							...
							if (!isFilterExclude && (widgetModuleId == filterModule.getModuleId()
									|| filterChildModuleIds.contains(widgetModuleId)
									|| isFilterFieldMappedToWidgetField))
