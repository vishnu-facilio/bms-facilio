...
public class MLServiceAction extends FacilioAction {
	...
	public String addMLModel() throws Exception {
		...
		LOGGER.info("STARTTIME ::"+this.modelInfo.getStartTime());

		endTime = DateTimeUtil.getCurrenTime();
		startTime = endTime - (90 * DAYS_IN_MILLISECONDS);

		if(this.modelInfo.getStartTime() != 0) { //for development purpose
			startTime = this.modelInfo.getStartTime();
			endTime = this.modelInfo.getEndTime();
		}
