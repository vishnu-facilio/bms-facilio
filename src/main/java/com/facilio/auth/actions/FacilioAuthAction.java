...
import com.facilio.accounts.commands.AuthenticationTransactionFactory;
...
public class FacilioAuthAction extends FacilioAction {
    ...
    private String addFacilioUser(String username, String password, String emailaddress) throws Exception {
        ...
        long orgId = OrgUtil.signUpOrg(signupInfo);
        if(orgId > 0) {
        ...
        if(getUsername() != null && getPassword() != null) {
            try {
                ...
                if (portalId() > 0) {
                    ...
                    validPassword = AuthUtill.verifyPasswordv2(getUsername(), getPassword());
                }
                ...
                if (validPassword == null) {
                    ...
                    if (validPassword) {
                        ...
                        if(portalUser) {
                            ...
                            user = AuthUtill.getUserBean().getFacilioUserv2(getUsername());
                        }
                        if(user == null) {
                ...
                User user = AuthUtill.getUserBean().getFacilioUserv2(getUsername());
                if(user != null ) {
...
//				} else {
				...
				if (validPassword == null) {
					...
					if (validPassword) {
						User user;
						// if (portalUser) {
						// user = AuthUtill.getUserBean().getPortalUserv2(getUsername(), portalId());
						// } else {
						user = AuthUtill.getUserBean().getFacilioUserv2(getUsername());
						// }
						if (user == null) {
				...
				String jwt = CognitoUtil.createJWT("id", "auth0", getUsername(),
						System.currentTimeMillis() + 24 * 60 * 60000, portalUser);
				LOGGER.info("Response token is " + jwt);
				setJsonresponse("token", jwt);
				setJsonresponse("username", getUsername());
				...
				if (user != null) {
					...
					userAgent = userAgent != null ? userAgent : "";
					String ipAddress = request.getHeader("X-Forwarded-For");
					ipAddress = (ipAddress == null || "".equals(ipAddress.trim())) ? request.getRemoteAddr()
							: ipAddress;
//		                    String userType = (AccountUtil.getCurrentAccount().isFromMobile() ? "mobile" : "web");
					String userType = "web";
					AuthUtill.getUserBean().startUserSessionv2(uid, getUsername(), jwt, ipAddress, userAgent, userType);
				}
			} catch (Exception e) {
				LOGGER.log(Level.INFO, "Exception while validating password, ", e);
				setJsonresponse("message", "Error while validating user name and password");
