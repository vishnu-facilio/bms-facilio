    ...
    public boolean executeCommand(Context context) throws Exception {
        ...
        for (QuestionType type : QuestionType.values()) {
            if (type.isOperatorSupportedForRules()) {
                questionVsOperators.put(type.name(), Objects.requireNonNull(filterOperatorsMap.get(type.getAnswerFieldType().name()), MessageFormat.format("Operators of type {0} cannot be null", type)));
            }
        }
