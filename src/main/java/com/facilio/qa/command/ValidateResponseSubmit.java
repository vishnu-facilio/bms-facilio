...
public class ValidateResponseSubmit extends FacilioCommand {
    ...
    private void validateResponseComplete (ResponseContext response) throws Exception {
        List<QuestionContext> questions = QAndAUtil.getChildRecordsFromTemplate(FacilioConstants.QAndA.QUESTION, response.getId());
        if (CollectionUtils.isNotEmpty(questions)) {
            List<AnswerContext> answers = QAndAUtil.getChildRecordsFromTemplate(FacilioConstants.QAndA.ANSWER, response.getId());
