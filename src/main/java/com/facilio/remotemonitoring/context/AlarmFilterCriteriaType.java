...
public enum AlarmFilterCriteriaType implements FacilioStringEnum {
    ALARM_OPEN_FOR_SPECIFIED_DURATION("Alarm open for specified duration", ImmutableMap.of(AlarmStrategy.RETURN_TO_NORMAL, new AlarmOpenForDurationOfTimeRTNHandler())),
    ...
    @Getter
    @Setter
    private Map<AlarmStrategy, AlarmCriteriaHandler> handlerMap;
    ...
    AlarmFilterCriteriaType(String displayName, Map<AlarmStrategy, AlarmCriteriaHandler> handlerMap) {
    ...
    public AlarmCriteriaHandler getHandler(RawAlarmContext rawAlarm) {
        if (MapUtils.isNotEmpty(handlerMap)) {
            return handlerMap.get(rawAlarm.getStrategyEnum());
        }
        return null;
    }
}