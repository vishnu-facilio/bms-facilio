...
import com.facilio.bmsconsoleV3.context.invoice.*;
...
public class InvoiceAPI {
    ...
    public static Double getDefaultorStoredMarkupValue(Context context, Double lineItemMarkup) throws Exception {
        ...
        if(invoiceSetting != null && invoiceSetting.getMarkupdisplayMode() == 1) {
            markupValue = invoiceSetting.getGlobalMarkupValue();
        }
        else if(invoiceSetting != null && invoiceSetting.getMarkupdisplayMode() == 2 && lineItemMarkup != null) {
            return lineItemMarkup;
        }
        else if(invoiceSetting != null && invoiceSetting.getMarkupDefaultValue() != null && invoiceSetting.getCanShowMarkupDefaultValue()) {
            markupValue = invoiceSetting.getMarkupDefaultValue().doubleValue();
        }

        return markupValue;
    }

    public static  Double getMarkupValue(BaseLineItemContext lineItem, InvoiceContextV3 invoice, Boolean isGlobalMarkup, Context context) throws Exception {
...
//        Double quantity = lineItem.getQuantity();
    ...
    public static  Boolean showMarkupValue(Context context,InvoiceContextV3 invoice) throws Exception {
        /*
        ...
        */
        ...
        InvoiceSettingContext invoiceSetting = (InvoiceSettingContext) context.get(FacilioConstants.ContextNames.INVOICE_SETTING);
        if( app != null && invoiceSetting != null) {
            if ( (app.getLinkName().equals(FacilioConstants.ApplicationLinkNames.MAINTENANCE_APP) || app.getLinkName().equals(FacilioConstants.ApplicationLinkNames.FACILIO_MAIN_APP))) {
                if(invoice != null && invoice.getInvoiceType() != null && invoice.getInvoiceType() == 4) {
                    return invoiceSetting.getVendorInvoice();
                }
                else if(invoice != null && invoice.getInvoiceType() != null && (invoice.getInvoiceType() == 1 || invoice.getInvoiceType() == 2)) {
                    return invoiceSetting.getEnduserInvoice();
                }
                else if(invoiceSetting.getVendorInvoice() || invoiceSetting.getEnduserInvoice()) {
                    return true;
                }
            }
                ...
                return invoiceSetting.getAllowUserSeeMarkup();
            }
    ...
    public static void lineItemsCostCalculationsForInvoice(InvoiceContextV3 record, List lineItems, Context context) throws Exception {
  /*
        ...
        */
        ...
        if (CollectionUtils.isNotEmpty(lineItems)) {
            ...
            invoiceTotalCost = lineItemsSubtotal + totalTaxAmount;
            if (record.getDiscountPercentage() != null) {
                Double discountAmount = (invoiceTotalCost * record.getDiscountPercentage() / 100);
                record.setDiscountAmount(discountAmount);
                invoiceTotalCost = invoiceTotalCost - discountAmount;
            } else if (record.getDiscountAmount() != null) {
                invoiceTotalCost -= record.getDiscountAmount();
            }

            if (Objects.equals(discountMode, 1l) && Objects.equals(taxMode, 2l)) {
                if (lookupValueIsNotEmpty(record.getTax())) {
                    TaxContext tax = getTaxDetails(record.getTax().getId());
                    totalTaxAmount = (invoiceTotalCost * tax.getRate()) / 100;
                    record.setTotalTaxAmount(totalTaxAmount);
                    invoiceTotalCost += totalTaxAmount;
                }
            }
...
//                invoiceTotalCost += totalMarkup;
            }

            record.setTotalCost(invoiceTotalCost);
    ...
    public static void setLineItems(InvoiceContextV3 invoice) throws Exception {
        ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
        String lineItemModuleName = FacilioConstants.ContextNames.INVOICE_LINE_ITEMS;
        ...
        SelectRecordsBuilder<InvoiceLineItemsContext> builder = new SelectRecordsBuilder<InvoiceLineItemsContext>()
                .moduleName(lineItemModuleName)
                .select(fields)
                .beanClass(InvoiceLineItemsContext.class)
                .andCondition(CriteriaAPI.getCondition(fieldsAsMap.get("invoice"), String.valueOf(invoice.getId()), NumberOperators.EQUALS))
                .fetchSupplements(fetchSupplementsList);
        List<InvoiceLineItemsContext> list = builder.get();
        if (CollectionUtils.isNotEmpty(list)) {
            invoice.setLineItems(list);
            Map<String, Object> currencyInfo = CurrencyUtil.getCurrencyInfo();
            for(InvoiceLineItemsContext lineItem : list){
                CurrencyUtil.checkAndFillBaseCurrencyToRecord(lineItem, currencyInfo);
            }
        }
    }

    //Need to change to invoice
    public static void setInvoiceAssociatedTerms(InvoiceContextV3 invoice) throws Exception {
        ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
        FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.INVOICE_ASSOCIATED_TERMS);
        ...
        SelectRecordsBuilder<InvoiceAssociatedTermsContext> builder = new SelectRecordsBuilder<InvoiceAssociatedTermsContext>()
                .module(module)
                .beanClass(InvoiceAssociatedTermsContext.class)
                .select(fields)
                .andCondition(CriteriaAPI.getCondition(fieldsAsMap.get("invoice"), String.valueOf(invoice.getId()), NumberOperators.EQUALS))
                .fetchSupplement(termsField);
        List<InvoiceAssociatedTermsContext> list = builder.get();
        if (CollectionUtils.isNotEmpty(list)) {
            invoice.setTermsAssociated(list);
        }
    }
    ...
    public static void addLocation(InvoiceContextV3 invoice, LocationContext location) throws Exception {

        if (location != null) {
            if (location.getId() > 0) {
                if (lookupValueIsNotEmpty(invoice.getTenant())) {
                    V3TenantContext tenant = V3TenantsAPI.getTenant(invoice.getTenant().getId());
                    location.setName(tenant.getName());
                }  else {
                    location.setName(invoice.getSubject() + "_location");
                }
    ...
    public static void addInvoiceTerms(Long id, List<InvoiceAssociatedTermsContext> termsAssociated) throws Exception {
        if (CollectionUtils.isNotEmpty(termsAssociated)) {
            InvoiceContextV3 invoice = new InvoiceContextV3();
            invoice.setId(id);
            for (InvoiceAssociatedTermsContext term : termsAssociated) {
    ...
    public static void setTaxSplitUpforInvoice(InvoiceContextV3 record, List lineItems, Context context) throws Exception {
    ...
    public static long getInvoiceCount(long workorderID) throws Exception {

        ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
        FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.INVOICE);
        List<FacilioField> fields = modBean.getAllFields(FacilioConstants.ContextNames.INVOICE);
        Map<String, FacilioField> fieldsMap = FieldFactory.getAsMap(fields);
        SelectRecordsBuilder<InvoiceContextV3> builder = new SelectRecordsBuilder<InvoiceContextV3>()
                .module(module)
                .beanClass(InvoiceContextV3.class)
                .select(fields)
                .andCondition(CriteriaAPI.getCondition(fieldsMap.get("workorder"), String.valueOf(workorderID), PickListOperators.IS));
        List<InvoiceContextV3> records = builder.get();
        return records.size();
    }

    public static void validateForWorkorder(InvoiceContextV3 invoice) throws Exception {
        // Considering one Invoice for a Workorder.
        if (lookupValueIsNotEmpty(invoice.getWorkorder())) {
            ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
            FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.INVOICE);
            List<FacilioField> fields = modBean.getAllFields(FacilioConstants.ContextNames.INVOICE);
            Map<String, FacilioField> fieldsMap = FieldFactory.getAsMap(fields);
            SelectRecordsBuilder<InvoiceContextV3> builder = new SelectRecordsBuilder<InvoiceContextV3>()
                    ...
                    .beanClass(InvoiceContextV3.class)
                    .select(fields)
                    .andCondition(CriteriaAPI.getCondition(fieldsMap.get("workorder"), String.valueOf(invoice.getWorkorder().getId()), PickListOperators.IS))
                    .andCondition(CriteriaAPI.getCondition(fieldsMap.get("isInvoiceRevised"), String.valueOf(false) ,BooleanOperators.IS));
            List<InvoiceContextV3> records = builder.get();
            if (CollectionUtils.isNotEmpty(records)) {
                InvoiceContextV3 firstRecord = records.get(0);
                if (invoice.getId() < 0 || (/* To allow update */ firstRecord.getId() != invoice.getId())) {
                    throw new RESTException(ErrorCode.VALIDATION_ERROR, "Invoice Already exist for this Workorder Invoice Id " + firstRecord.getParentId());
                }
            }
        }
    }
    ...
    public static List<TermsAndConditionContext> fetchInvoiceDefaultTerms() throws Exception {
        ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
        FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.TERMS_AND_CONDITIONS);
        List<FacilioField> fields = modBean.getAllFields(module.getName());
        ...
        SelectRecordsBuilder<TermsAndConditionContext> builder = new SelectRecordsBuilder<TermsAndConditionContext>()
                ...
                .beanClass(TermsAndConditionContext.class)
                .select(fields)
                .andCondition(CriteriaAPI.getCondition("DEFAULT_ON_INVOICE", "defaultOnInvoice", String.valueOf(1), NumberOperators.EQUALS))
                ;
        ;
    ...
    public static void updateTermsAssociated(List<InvoiceAssociatedTermsContext> associatedTerms) throws Exception {
    ...
    public static InvoiceSettingContext getInvoiceSetting() throws Exception {

        GenericSelectRecordBuilder selectBuilder = new GenericSelectRecordBuilder()
                .select(FieldFactory.getInvoicSettingFields())
        ...
        if (props != null && !props.isEmpty()) {
            InvoiceSettingContext invoiceSettingDATA = FieldUtil.getAsBeanFromMap(props.get(0), InvoiceSettingContext.class);

            return invoiceSettingDATA;
        }
        return null;
    }

    public static List<InvoiceLineItemsContext> getInvoiceLineItems(Long invoiceId) throws Exception {
        ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
        FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.INVOICE_LINE_ITEMS);
        List<FacilioField> fields = modBean.getAllFields(module.getName());
        ...
        SelectRecordsBuilder<InvoiceLineItemsContext> builder = new SelectRecordsBuilder<InvoiceLineItemsContext>()
                ...
                .beanClass(InvoiceLineItemsContext.class)
                .select(fields)
                .andCondition(CriteriaAPI.getCondition("INVOICE_ID", "invoice", String.valueOf(invoiceId), NumberOperators.EQUALS))
                ;
        try {
            List<InvoiceLineItemsContext> list = builder.get();
            return list;
    ...
    public static List<InvoiceContextV3> getInvoiceFromGroupId(long groupId) throws Exception{
        ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
        FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.INVOICE);
        ...
        SelectRecordsBuilder<InvoiceContextV3> builder = new SelectRecordsBuilder<InvoiceContextV3>()
                ...
                .orderBy(fieldMap.get("invoiceVersion").getCompleteColumnName()+ " DESC")
                .fetchSupplement((LookupField) fieldMap.get("moduleState"));
        return builder.get();
    }
    public static Map<String,Object> getInvoiceGroupAndVersion(long invoiceId) throws Exception{
        ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
        FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.INVOICE);
        Map<String, FacilioField> fieldMap = FieldFactory.getAsMap(modBean.getAllFields(module.getName()));
        ...
        SelectRecordsBuilder<InvoiceContextV3> builder = new SelectRecordsBuilder<InvoiceContextV3>()
                ...
                .select(fieldList)
                .beanClass(InvoiceContextV3.class)
                .andCondition(CriteriaAPI.getIdCondition(invoiceId,module));
        return builder.getAsProps().get(0);
    }
    public static Long getInvoiceIdFromGroupAndVersion(long groupId, long version) throws Exception{
        ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
        FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.INVOICE);
        List<FacilioField> fieldList = modBean.getAllFields(FacilioConstants.ContextNames.INVOICE);
        ...
        SelectRecordsBuilder<InvoiceContextV3> builder = new SelectRecordsBuilder<InvoiceContextV3>()
                ...
                .beanClass(InvoiceContextV3.class)
                .andCondition(CriteriaAPI.getCondition(fieldMap.get("group"),String.valueOf(groupId),NumberOperators.EQUALS))
                .andCondition(CriteriaAPI.getCondition(fieldMap.get("invoiceVersion"),String.valueOf(version),NumberOperators.EQUALS));
        return builder.fetchFirst().getId();
    }

    public static InvoiceContextV3 getInvoice(long id) throws Exception {
        ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
        FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.INVOICE);
        Map<String, FacilioField> fieldMap = FieldFactory.getAsMap(modBean.getAllFields(module.getName()));
        InvoiceContextV3 invoice = (InvoiceContextV3) V3RecordAPI.getRecord(module.getName(), id, InvoiceContextV3.class);
        InvoiceAPI.setLineItems(invoice);
        InvoiceAPI.setInvoiceAssociatedTerms(invoice);
        return invoice;
    }

    public static int updateInvoice(InvoiceContextV3 invoiceContextV3) throws Exception{
        ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
        FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.INVOICE);

        UpdateRecordBuilder<InvoiceContextV3> builder = new UpdateRecordBuilder<InvoiceContextV3>()
                .module(module)
                .fields(modBean.getAllFields(module.getName()))
                .andCondition(CriteriaAPI.getIdCondition(invoiceContextV3.getId(),module));
        return builder.update(invoiceContextV3);
    }

    public static void updateTermsAssociatedV3(List<InvoiceAssociatedTermsContext> associatedTerms) throws Exception {
        if(CollectionUtils.isNotEmpty(associatedTerms)) {
            ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
            FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.INVOICE_ASSOCIATED_TERMS);
            List<FacilioField> fields = modBean.getAllFields(module.getName());
    ...
    public static void setInvoiceApprovers(InvoiceContextV3 invoice) throws Exception {
        ModuleBean modBean = (ModuleBean) BeanFactory.lookup("ModuleBean");
        FacilioModule module = modBean.getModule(FacilioConstants.ContextNames.INVOICE_APPROVERS_REL);
        List<FacilioField> fields = modBean.getAllFields(module.getName());
        Map<String, FacilioField> fieldsAsMap = FieldFactory.getAsMap(fields);

        SelectRecordsBuilder<InvoiceApproversContext> builder = new SelectRecordsBuilder<InvoiceApproversContext>()
                .module(module)
                .beanClass(InvoiceApproversContext.class)
                .select(fields)
                .andCondition(CriteriaAPI.getCondition(fieldsAsMap.get("left"), String.valueOf(invoice.getId()), NumberOperators.EQUALS));

        List<InvoiceApproversContext> list = builder.get();
        List<V3PeopleContext> peopleContextList = new ArrayList<>();
        if (CollectionUtils.isNotEmpty(list)) {
            for(InvoiceApproversContext approversContext : list)
            {
                peopleContextList.add(approversContext.getRight());
            }
            invoice.setApprovers(peopleContextList);
        }
    }
}
