        ...
        if(CollectionUtils.isNotEmpty(requestForQuotations)){
            for(V3RequestForQuotationContext requestForQuotation : requestForQuotations) {
                if ( MapUtils.isNotEmpty(bodyParams) && bodyParams.containsKey("rfqFinalized") && (boolean) bodyParams.get("rfqFinalized")) {
                    ...
                    chain.getContext().put(FacilioConstants.ContextNames.REQUEST_FOR_QUOTATION,requestForQuotation);
                    chain.getContext().put(FacilioConstants.ContextNames.REQUEST_FOR_QUOTATION_LINE_ITEMS,requestForQuotation.getRequestForQuotationLineItems());
                    chain.execute();
                }
        /*
