...
public class AssociateClientFromSite extends FacilioCommand {
    @Override
    public boolean executeCommand(Context context) throws Exception {
        Map<String, Object> bodyParams = Constants.getBodyParams(context);
        if(bodyParams != null){
            Boolean isAssociateSite = (Boolean) bodyParams.get("isAssociateSites");
            if (isAssociateSite != null && isAssociateSite) {
                Map<String, List> recordMap = (Map<String, List>) context.get(Constants.RECORD_MAP);
                String moduleName = Constants.getModuleName(context);
                Long clientId = (Long) context.get("recordId");
                if (moduleName != null && !moduleName.isEmpty() && recordMap != null && MapUtils.isNotEmpty(recordMap)) {
                    ...
                    if (records != null && !records.isEmpty()) {
                        ...
                        if (siteIds != null && !siteIds.isEmpty() && clientId > 0) {
