...
public class ValidateFacilityBookingCommandForEditV3 extends FacilioCommand {
        ...
        if(CollectionUtils.isNotEmpty(bookings)) {
            ...
            if (MapUtils.isEmpty(bodyParams) || (!bodyParams.containsKey("cancel") && !bodyParams.containsKey("cancelBooking"))) {
                ...
                for (V3FacilityBookingContext booking : bookings) {
                    ...
                    List<BookingSlotsContext> slotList = new ArrayList<>();
                    if (MapUtils.isEmpty(subformMap) || !subformMap.containsKey(FacilioConstants.ContextNames.FacilityBooking.BOOKING_SLOTS)) {
                        slotList = FacilityAPI.getBookingSlots(booking.getId());
                    }
                    else {
                       slotList = FieldUtil.getAsBeanListFromMapList(subformMap.get(FacilioConstants.ContextNames.FacilityBooking.BOOKING_SLOTS), BookingSlotsContext.class);
                    }
...
//                                }
                    ...
                    if (CollectionUtils.isNotEmpty(booking.getFacilityBookingExternalAttendee())) {
                        ...
                        List<V3ExternalAttendeeContext> externalAttendees = FacilityAPI.getExternalAttendees(booking.getId());
