...
//		addScopingMaintenanceApp(inspection,applicationScopingId);
	...
	public void addDefaultFormForInspectionTemplateMaintenanceApp(FacilioModule inspection) throws Exception {
		...
		configSection.setSequenceNumber(2);
		sections.add(configSection);
		FormSection triggerSection = new FormSection();
		triggerSection.setName("Triggers");
		triggerSection.setSectionType(FormSection.SectionType.FIELDS);
		triggerSection.setShowLabel(false);
		triggerSection.setVersion(Version.V2.getVersionId());
		List<FormField> triggerFields = new ArrayList<>();
		seq = 0;
		FormField triggerField = (new FormField(-1, "triggers", FieldDisplayType.INSPECTION_TRIGGER, "Triggers", FormField.Required.OPTIONAL, ++seq, 1));
		triggerField.setVersion(Version.V2.getVersionId());
		triggerFields.add(triggerField);
		triggerSection.setFields(triggerFields);
		triggerSection.setSequenceNumber(3);
		sections.add(triggerSection);
		defaultForm.setSections(sections);
		defaultForm.setType(FacilioForm.Type.FORM);
		defaultForm.setAppId(ApplicationApi.getApplicationIdForLinkName(FacilioConstants.ApplicationLinkNames.MAINTENANCE_APP));
		defaultForm.setIsSystemForm(true);
		FormsAPI.createForm(defaultForm, inspection);
		Map<Long, FormField> formFieldMap = defaultForm.getSections().stream().map(FormSection::getFields)
				.flatMap(List::stream).filter(formField -> formField.getFieldId() != -1).collect(Collectors.toMap(FormField::getFieldId, Function.identity()));
		addSingleAssetSelectRule(defaultForm, fieldMap, formFieldMap);
		addMultipleAssetSelectRule(defaultForm, fieldMap, formFieldMap);
		addBuildingFilteringWithSiteRule(defaultForm, fieldMap, formFieldMap);
		addAssetCategoryShowRule(defaultForm, fieldMap, formFieldMap);
