package com.facilio.sandbox.utils;

public class SandboxConstants {
    public static final String SANDBOX = "sandbox";
    public static final String SANDBOX_ID = "sandboxId";
    public static final String SANDBOX_LIST = "sandboxList";
    public static final String SANDBOX_ACCOUNT = "sandboxAccount";
    public static final String SANDBOX_COUNT = "sandboxCount";
    public static final String SANDBOX_ORG = "sandboxOrg";
    public static final String SANDBOX_ORG_USER = "sandboxOrgUser";
}
