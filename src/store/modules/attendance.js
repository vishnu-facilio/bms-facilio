...
const actions = {
  ...
  fetchAttendanceList: ({ commit, dispatch, state }, payload) => {
    let params = ''
    if (payload.page) {
    ...
    let url = '/v2/attendance/view/' + payload.viewname + '?' + params
    return http.get(url).then(function(response) {
      commit('SET_ATTENDANCE', response.data.result.attendance)
    })
  },
  reload: ({ commit, dispatch, state }) => {
    let url = `/v2/attendance/view/${state.currentView}?page=${state.currentPage}&perPage=${state.perPage}`
