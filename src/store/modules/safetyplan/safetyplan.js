...
const actions = {
  ...
  fetchRecords: ({ commit, state }, payload) => {
    let params = ''
    if (payload.page) {
    ...
    return http.get(url).then(function(response) {
      commit('SET_RECORDS', response.data.result.safetyPlans)
    })
  },
  fetchRecordDetails({ commit, state }, payload) {
    return http
      .get(`v2/${state.requestUrl}/details` + '?recordId=' + payload.id)
      .then(function(response) {
        commit('SET_RECORD_DATA', response.data.result.safetyPlan)
      })
  },
  reload: ({ commit, state }) => {
