...
const actions = {
  ...
  fetchPurchaseOrders: ({ commit, dispatch, state }, payload) => {
    let params = ''
    if (payload.page) {
    ...
    let url = '/v2/purchaseorder/' + payload.viewname + '?' + params
    return http.get(url).then(function(response) {
      commit('SET_PURCHASE_ORDER', response.data.result.purchaseorders)
    })
  },
  reload: ({ commit, dispatch, state }) => {
    let url = `/v2/purchaseorder/${state.currentView}?page=${state.currentPage}&perPage=${state.perPage}`
