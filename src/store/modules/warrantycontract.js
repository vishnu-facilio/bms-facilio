...
const actions = {
  ...
  fetchWarrantyContracts: ({ commit, dispatch, state }, payload) => {
    let params = ''
    if (payload.page) {
    ...
    let url = '/v2/warrantycontract/' + payload.viewname + '?' + params
    return http.get(url).then(function(response) {
      commit('SET_WARRANTY_CONTRACT', response.data.result.warrantycontracts)
    })
  },
  reload: ({ commit, dispatch, state }) => {
    let url = `/v2/warrantycontract/${state.currentView}?page=${state.currentPage}&perPage=${state.perPage}`
...
const mutations = {
  ...
  SET_WARRANTY_CONTRACT: (state, payload) => {
    if (state.isNew) {
      state.warrantyContracts =
      ...
      state.warrantyContracts =
        state.perPage && state.currentPage > 1
          ? [...state.warrantyContracts, ...payload]
          : payload
      state.canLoadMore = payload.length === state.perPage
    }
  },
