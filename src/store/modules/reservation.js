...
const actions = {
  ...
  fetchReservations: ({ commit, dispatch, state }, payload) => {
    let params = ''
    if (payload.page) {
    ...
    let url = '/v2/reservations/view/' + payload.viewname + '?' + params
    return http.get(url).then(function(response) {
      commit('SET_RESERVATIONS', response.data.result.reservations)
    })
  },
  reload: ({ commit, dispatch, state }) => {
    let url = `/v2/reservations/view/${state.currentView}?page=${state.currentPage}&perPage=${state.perPage}`
