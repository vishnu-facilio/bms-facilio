...
const actions = {
  ...
  fetchRentalLeaseContracts: ({ commit, dispatch, state }, payload) => {
    let params = ''
    if (payload.page) {
    ...
    let url = '/v2/rentalleasecontract/' + payload.viewname + '?' + params
    return http.get(url).then(function(response) {
      commit(
        'SET_RENTAL_LEASE_CONTRACT',
        response.data.result.rentalleasecontracts
      )
    })
  },
  reload: ({ commit, dispatch, state }) => {
    let url = `/v2/rentalleasecontract/${state.currentView}?page=${state.currentPage}&perPage=${state.perPage}`
...
const mutations = {
  ...
  SET_RENTAL_LEASE_CONTRACT: (state, payload) => {
    if (state.isNew) {
      state.rentalLeaseContracts =
      ...
      state.rentalLeaseContracts =
        state.perPage && state.currentPage > 1
          ? [...state.rentalLeaseContracts, ...payload]
          : payload
      state.canLoadMore = payload.length === state.perPage
    }
  },
