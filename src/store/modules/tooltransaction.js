...
const actions = {
  ...
  fetchToolTransactions: ({ commit, dispatch, state }, payload) => {
    let params = ''
    if (payload.page) {
    ...
    let url = '/v2/toolTransactions/view/' + payload.viewname + '?' + params
    return http.get(url).then(function(response) {
      commit('SET_TOOLTRANSACTIONS', response.data.result.toolTransactions)
    })
  },
  reload: ({ commit, dispatch, state }) => {
    let url = `/v2/toolTransactions/view/${state.currentView}?page=${state.currentPage}&perPage=${state.perPage}`
...
const mutations = {
  ...
  SET_TOOLTRANSACTIONS: (state, payload) => {
    if (state.isNew) {
      state.toolTransactions =
      ...
      state.toolTransactions =
        state.perPage && state.currentPage > 1
          ? [...state.toolTransactions, ...payload]
          : payload
      state.canLoadMore = payload.length === state.perPage
    }
  },
