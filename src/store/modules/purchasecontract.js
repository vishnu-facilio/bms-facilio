...
const actions = {
  ...
  fetchPurchaseContracts: ({ commit, dispatch, state }, payload) => {
    let params = ''
    if (payload.page) {
    ...
    let url = '/v2/purchasecontract/' + payload.viewname + '?' + params
    return http.get(url).then(function(response) {
      commit('SET_PURCHASE_CONTRACT', response.data.result.purchasecontracts)
    })
  },
  reload: ({ commit, dispatch, state }) => {
    let url = `/v2/purchasecontract/${state.currentView}?page=${state.currentPage}&perPage=${state.perPage}`
...
const mutations = {
  ...
  SET_PURCHASE_CONTRACT: (state, payload) => {
    if (state.isNew) {
      state.purchaseContracts =
      ...
      state.purchaseContracts =
        state.perPage && state.currentPage > 1
          ? [...state.purchaseContracts, ...payload]
          : payload
      state.canLoadMore = payload.length === state.perPage
    }
  },
