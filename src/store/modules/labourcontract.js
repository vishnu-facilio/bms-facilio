...
const actions = {
  ...
  fetchLabourContracts: ({ commit, dispatch, state }, payload) => {
    let params = ''
    if (payload.page) {
    ...
    let url = '/v2/labourcontract/' + payload.viewname + '?' + params
    return http.get(url).then(function(response) {
      commit('SET_LABOUR_CONTRACT', response.data.result.labourcontracts)
    })
  },
  reload: ({ commit, dispatch, state }) => {
    let url = `/v2/labourcontract/${state.currentView}?page=${state.currentPage}&perPage=${state.perPage}`
...
const mutations = {
  ...
  SET_LABOUR_CONTRACT: (state, payload) => {
    if (state.isNew) {
      state.labourContracts =
