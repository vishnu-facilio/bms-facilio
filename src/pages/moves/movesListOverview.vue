                  ...
                  <el-row>
                    <el-col :span="24">
                      ...
                      <div
                        ...
                        :title="row[mainFieldKey].name || row[mainFieldKey]"
                        v-tippy="{
                          placement: 'top',
                      ...
                      >
                        {{ row[mainFieldKey].name || row[mainFieldKey] }}
                      </div>
                    </el-col>
                  </el-row>
                </div>
  ...
  methods: {
    ...
    switchView(view) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = `/app/mv/moves/${view.name}/${null}/overview`
    ...
    async loadData(loadMore, forceFetch) {
      ...
      if (error) {
        let { message = 'Error Occured while fetching Move list' } = error
        ...
        if (forceFetch) {
          ...
          if (loadMore) {
            this.moveList = [...(this.moveList || []), ...list]
          }
        }
        let listCount = this.$getProperty(meta, 'pagination.totalCount', null)
        this.canLoadMoreList = listCount > (this.moveList || []).length
      }
      this.loading = false
