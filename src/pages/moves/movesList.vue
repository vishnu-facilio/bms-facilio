...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Moves'
    },
    appliedFilters() {
      let {
        $route: { query },
    ...
    titleColumn() {
      let { viewColumns } = this
      if (!isEmpty(viewColumns)) {
        return viewColumns.find(column => column.name === 'employee')
      }
      return {}
    },
    moduleName() {
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(
          message || 'Error Occured while fetching moves list'
        )
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`space.moves.delete_move`),
        message: this.$t(`space.moves.delete_confirmation_move`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          let { message } = error
          this.$message.error(message || 'Error Occured while deleting move')
          this.loading = false
        } else {
          this.$message.success(this.$t(`space.moves.delete_success_move`))
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-move' })
      }
    },
    editRecord({ id = null } = {}) {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'edit-move', params: { id } })
      }
    },
