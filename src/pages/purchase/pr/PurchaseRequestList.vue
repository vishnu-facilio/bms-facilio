      ...
      <template v-else>
        <div class="cm-list-container">
          ...
          >
            ...
            <div
              v-if="convertPrVisibility()"
              class="action-btn-slide btn-block"
            >
              <button
                class="btn btn--tertiary"
                @click="convertPr('purchaseorder')"
              >
                <div>
                  {{ $t('common.products.convert_prs_to_po') }}
                </div>
              </button>
            </div>
            <div
            ...
            >
              <button
                class="btn btn--tertiary"
                @click="convertPr('requestForQuotation')"
              >
                <div>
                  {{ $t('common.products.convert_prs_to_rfq') }}
                </div>
              </button>
            </div>
            <CustomButton
...
export default {
  ...
  methods: {
    ...
    convertPrVisibility() {
      let vendorid = null
      for (let i in this.selectedListItemsIds) {
        if (
          this.selectedListItemsIds[i].vendor &&
          this.selectedListItemsIds[i].vendor.id
        ) {
          if (vendorid !== null) {
            if (vendorid === this.selectedListItemsIds[i].vendor.id) {
              continue
            } else {
              return false
            }
          } else {
            vendorid = this.selectedListItemsIds[i].vendor.id
          }
        }
      }
      return true
