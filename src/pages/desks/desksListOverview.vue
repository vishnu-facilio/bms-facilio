  ...
  methods: {
    ...
    switchView(view) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = `/app/de/desks/${view.name}/${null}/overview`
    ...
    async loadData(loadMore, forceFetch) {
      ...
      if (error) {
        let { message = 'Error Occured while fetching Desk list' } = error
        ...
        this.canLoadMoreList = listCount > (this.deskList || []).length
      }
      this.loading = false
