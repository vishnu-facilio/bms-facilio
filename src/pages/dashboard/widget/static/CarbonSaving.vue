  ...
  <div v-else>
    <div class="dragabale-card h100 row" v-if="!loading">
      ...
      <div class="col-12 h50 carbon-weather text-left" style="height:52%;">
        <div class="pL20 weather-layout" style="padding-top:15px;">
          <div>
          ...
          <div class="area uppercase">
            {{ $t('panel.card.carbon_savings') }}
          </div>
        </div>
      </div>
    </div>
    ...
    initData() {
      ...
      self.$http
        ...
        .then(function(response) {
          if (response.data.cardResult.weather) {
          ...
          self.data.carbon = self.prepareEmission(
            response.data.cardResult.carbonSaving
          )
          self.loading = false
          self.demoMode = false
          self.wloading = false
