<template>
  ...
  <div class="row list-view-widget" v-else>
    ...
    <div class="col table-scroll fList-widget-page">
      <table :class="['fc-list-view-table', zoomClass]" ref="f-list-table">
        <thead v-if="!isPrinting || (records && records.length)">
          <tr>
            <th
...
export default {
  ...
  methods: {
    ...
    initRecords(v2api, urlx) {
      ...
      if (self.dbFilterJson || self.dbTimelineFilter) {
        //for list widget ,combine dbuser and dbtimeline filters
        let userFilterObj = self.dbFilterJson
        let timelineFilterObj = null
        if (self.dbTimelineFilter) {
          timelineFilterObj = {
            [self.dbTimelineFilter.dateField]: {
              operatorId: 20,
              value: [
                String(self.dbTimelineFilter.startTime),
                String(self.dbTimelineFilter.endTime),
              ],
            },
          }
        }
    ...
    getSummaryUrl(row) {
      ...
      if (isWebTabsEnabled()) {
        ...
        if (this.config.widget.dataOptions.moduleName === 'workorder') {
          ...
          this.config.widget.dataOptions.moduleName === 'groupinvite'
        ) {
          return '/app/vi/invites/all/summary/' + id
...
.list-view-widget .table-scroll {
  height: 100%;
  padding-bottom: 30px;
}

.list-view-widget thead th {
...
.list-view-widget .fc-list-view-table tbody tr:nth-child(even) {
...
@media print {
  ...
  .list-view-widget table {
    border: none !important;
    page-break-inside: avoid;
  }

  .list-view-widget .list-print-header {
    display: block;
    padding: 10px;
    font-size: 12px;
  }
  ...
  .list-view-widget tbody td {
    word-break: break-all;
    padding-left: 14px;
    page-break-inside: avoid;
    page-break-after: auto;
  }
  .list-view-widget tbody td:first-child {
    word-break: break-all;
    padding-right: 40px;
  }
}
