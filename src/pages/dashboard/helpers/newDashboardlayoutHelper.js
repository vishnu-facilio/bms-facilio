...
export default {
  ...
  methods: {
    ...
    deletedashboard(dashboard, index, idx) {
      ...
      let text = this.$t('common._common.delete_this_dashboard')
      if (dashboard?.rawdata?.isTabPresent) {
        text = `${this.$t('common._common.dashboard_deletion_with_tab')} ${
          dashboard.label
        }?`
      }
      self.$dialog
    ...
    dashboardCommand(modeule) {
      ...
      } else if (modeule == 'editFilters') {
        eventBus.$emit('editFilterClicked')
      } else if (modeule === 'openClonePopup') {
        this.isClone = true
        this.dashboardNameObj.name = this.dashboard.label
      } else if (modeule === 'clone') {
        if (
      ...
      } else if (modeule === 'enableDarkMode') {
        this.switchTheme()
      }
    },
    handleCommand(command, thisContext, cancelRedirect) {
      ...
      if (this.subheaderMenu.length) {
        let query = helpers.cloneObject(this.query)
        if ((query || {}).tab) {
          delete query.tab
        }
        if (
    ...
    EnableMobileDashboard() {
      ...
      http.post('/dashboard/updateDashboard', params).then(function() {
    ...
    loadDashboards(disableRedirect, appId) {
      ...
      // if (!isEmpty(appId)) {
      //   url += '?appId=' + appId
      //   this.cloned_app_id = appId
      // }
      this.cloned_app_id = !isEmpty(appId) ? appId : getApp().id
      let url = 'v3/dashboard/list'
      let params = {
        appId: !isEmpty(appId) ? appId : getApp().id,
        withSharing: true,
      ...
      let self = this
      self.loading = true
      API.get(url, params, { uniqueKey: 'get-dashboard-list' })
        .then(function(response) {
    ...
    getsubheaderchildren(list) {
      ...
      if (list.length) {
        list.forEach(rt => {
          ...
          let copyObj = helpers.cloneObject(listobj)
          if (
    ...
    redirectClonedDashboardToEdit(dashboard_link_name) {
      if (isWebTabsEnabled()) {
        ...
        let link =
          '/app/home/editdashboard/' + dashboard_link_name + '?create=edit'
    ...
    async cloneDashboardToPortal(isCloneToAnotherPortal) {
      ...
      if (
        ...
        params = {
          ...
          target_app_id: dashboardObj.selectedAppId,
