...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Parking Stall'
    },
    appliedFilters() {
      let {
        $route: { query },
    ...
    filteredViewColumns() {
      ...
      let { fixedColumns, fixedSelectableColumns } =
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(
          message || 'Error Occured while fetching Parking Stall list'
        )
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`space.rooms.delete_room`),
        message: this.$t(`space.rooms.delete_confirmation_room`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          let { message } = error
          this.$message.error(
            message || 'Error Occured while deleting Parking Stall'
          )
          this.loading = false
        } else {
          this.$message.success(this.$t(`space.rooms.delete_success_room`))
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-rooms' })
      }
    },
    editRecord({ id = null } = {}) {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'edit-rooms', params: { id } })
      }
    },
