        ...
        >
          ...
          >
            ...
            <div class="q-item-label nowo-label">
              {{ $t('space.lockers.no_data_lockers') }}
            </div>
          </div>
          <template v-else>
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Lockers'
    },
    appliedFilters() {
      let {
        $route: { query },
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(
          message || 'Error Occured while fetching lockers list'
        )
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`space.lockers.delete_locker`),
        message: this.$t(`space.lockers.delete_confirmation_locker`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          let { message } = error
          this.$message.error(message || 'Error Occured while deleting locker')
          this.loading = false
        } else {
          this.$message.success(this.$t(`space.lockers.delete_success_locker`))
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-locker' })
      }
    },
    editRecord({ id = null } = {}) {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'edit-locker', params: { id } })
      }
    },
