  ...
  methods: {
    ...
    switchView(view) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = `/app/lo/lockers/${view.name}/${null}/overview`
    ...
    async loadData(loadMore, forceFetch) {
      ...
      if (error) {
        let { message = 'Error Occured while fetching Locker list' } = error
        ...
        if (forceFetch) {
          ...
          if (loadMore) {
            this.lockerList = [...(this.lockerList || []), ...list]
          }
        }
        let listCount = this.$getProperty(meta, 'pagination.totalCount', null)
        this.canLoadMoreList = listCount > (this.lockerList || []).length
      }
      this.loading = false
