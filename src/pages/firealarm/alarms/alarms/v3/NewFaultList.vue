  ...
  >
          ...
          <div class="cm-list-container">
            ...
            >
              <div class="action-btn-slide btn-block">
                <button
                  v-if="$hasPermission('newreadingalarm:DELETE')"
            ...
            >
              ...
              <template #[slotList[3].criteria]="{ record }">
                <div style="min-width: 150px">
                  <div
                    v-if="$hasPermission('newreadingalarm:ACKNOWLEDGE_ALARM')"
                    ...
                    <el-dropdown-item
                      ...
                      v-if="
                        isActiveAlarm(record) &&
                          $hasPermission('newreadingalarm:CREATE_WORKORDER')
                      "
                    >
              ...
              <template #[slotList[7].criteria]="{ record }">
                <div>
                  <span class="q-item-label f13">
                    {{ getCostImpact(record) }}
                  </span>
                </div>
              </template>
    ...
    <portal to="view-manager-link">
      <router-link
        tag="div"
        :to="{
          path: `/app/fa/faults/newreadingalarm/viewmanager`,
        }"
...
export default {
  ...
  computed: {
    slotList() {
      return [
        ...
        {
          criteria: JSON.stringify({ name: 'readingAlarmCategory' }),
        },
        {
        ...
        {
          criteria: JSON.stringify({ name: 'costImpact' }),
        },
        {
          criteria: JSON.stringify({ name: 'energyImpact' }),
        },
      ]
    },
    parentPath() {
  ...
  methods: {
    ...
    getAlarmCategory(record) {
      const alarmCategory = this.$getProperty(record, 'readingAlarmCategory')
      return alarmCategory && alarmCategory.id > -1
        ? this.$getProperty(alarmCategory, 'displayName')
        : '---'
    },
    getReadingFieldName(record) {
    ...
    getCostImpact(record) {
      if (record.hasOwnProperty('costImpact')) {
        let impact = this.$getProperty(record, 'costImpact', 0)
        return impact.toFixed(2)
      }
      return '---'
    },
    getEnergyImpact(record) {
      if (record.hasOwnProperty('energyImpact')) {
        let impact = this.$getProperty(record, 'energyImpact', 0)
        return impact.toFixed(2)
      }
      return '---'
    },
    ...
    async acknowledgeAlarm(record) {
      let dataObj = {
        ...
        occurrence: {
          id:
            record && record.lastOccurrenceId ? record.lastOccurrenceId : null,
        },
        acknowledged: true,
