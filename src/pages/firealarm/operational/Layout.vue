...
export default {
  data() {
    return {
      ...
      filterConfig: {
        moduleName: 'operationalarm',
        includeParentCriteria: true,
        path: '/app/fa/outofschedule/',
        data: {
          subject: {
            label: 'Message',
            displayType: 'string',
  ...
  computed: {
    ...
    viewModuleName() {
      ...
      return selectedView ? selectedView.moduleName : 'operationalarm'
    },
  },
  watch: {
  ...
  methods: {
    ...
    setDefaultView() {
      if (this.$route.path.endsWith('/fa/faults')) {
        if (this.views.length && this.views[0].views.length) {
          this.$router.push({
            path: '/app/fa/outofschedule/' + this.views[0].views[0].name,
    ...
    openChild(command) {
      ...
      if (a2) {
        self.$router.push({
          path: '/app/fa/outofschedule/' + a2[0].name,
        })
      }
    },
    savingView(command) {
                ...
                if (response.data === null) {
                  for (let i = 0; i < self.views.length; i++) {
                    ...
                    if (key1) {
                      ...
                      if (key2) {
                        self.$router.push('/app/fa/outofschedule/' + key2.name)
                        self.loadViews()
                        ...
                        if (key3) {
                          self.$router.push(
                            '/app/fa/outofschedule/' + a2[0].name
                          )
                          self.loadViews()
    ...
    getSubHeaderMenu() {
      ...
      if (a1.length > 0) {
        return a1.map(view => ({
          ...
          path: {
            path: '/app/fa/outofschedule/' + view.name,
          },
