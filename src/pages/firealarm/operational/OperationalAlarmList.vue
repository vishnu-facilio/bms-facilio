...
import NewAlarm from 'src/pages/firealarm/operational/OperationalSummaryOverview'
...
export default {
  ...
  computed: {
    ...
    columnConfig() {
      return getColumnConfig('operationalarm')
    },
  },
  watch: {
  ...
  methods: {
    ...
    openChild(viewNames) {
      let url = '/app/fa/outofschedule/' + viewNames + '/summary/' + null
        ...
        .then(function(response) {
          if (self.$store.state.newAlarm.alarms.length > 0) {
            ...
            let newPath = self.$route.path.substring(
              0,
              self.$route.path.indexOf('/outofschedule/')
            )
            newPath += '/outofschedule/' + viewNames
            self.$router.push({
              path: newPath,
              query: self.$route.query,
            })
