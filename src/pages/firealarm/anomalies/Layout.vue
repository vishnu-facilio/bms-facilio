+<template>
  <div class="layout container fc-anomalies-page">
    <subheader
          ...
          >
            ...
            >
              <img
                ...
                style="width: 19px;height: 19px;margin-left: 5px;margin-right: 5px;margin-top: 7px;"
...
export default {
    ...
    return {
      ...
      filterConfig: {
        ...
        data: {
          ...
          alarmType: {
            label: this.$t('alarm.alarm.alarm_category'),
            displayType: 'select',
            options: { 5: 'Energy', 6: 'Fire', 7: 'Hvac' },
            operatorId: 9,
            value: [],
          },
          space: {
            label: this.$t('alarm.alarm.space'),
            displayType: 'space',
            options: {},
            operatorId: 38,
            value: [],
  ...
  mounted() {
    this.loadViews()
    this.$store.dispatch('view/loadModuleMeta', 'mlAnomalyAlarm')
    this.getViewDetail()
  ...
  methods: {
    ...
    metaFieldsLists() {
      let self = this
      self.$http
        .get('/module/metafields?moduleName=' + 'mlAnomalyAlarm')
        .then(response => {
    ...
    openChild(command) {
      ...
      if (a2) {
        self.$router.push({
          path: '/app/fa/anomalies/' + a2[0].name,
        })
      }
    },
    savingView(command) {
                ...
                if (response.data === null) {
                  for (let i = 0; i < self.views.length; i++) {
                    ...
                    if (key1) {
                      ...
                      if (key2) {
                        self.$router.push('/app/fa/anomalies/' + key2.name)
                        self.loadViews()
                        ...
                        let a2 = key3.views
                        if (key3) {
                          self.$router.push('/app/fa/anomalies/' + a2[0].name)
                          self.loadViews()
    ...
    getSubHeaderMenu() {
      ...
      if (a1.length > 0) {
        return a1.map(view => ({
          ...
          path: {
            path: '/app/fa/anomalies/' + view.name,
          },
    ...
    exportList(type) {
      ...
      let url =
        '/exportModule?type=' +
        type +
        '&moduleName=mlAnomalyAlarm&viewName=' +
        this.currentView
      self.$message({
        showClose: true,
        message: 'Downloading...',
...
.fc-anomalies-page .page-sort-popover {
