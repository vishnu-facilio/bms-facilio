...
export default {
  ...
  props: ['formVisibility', 'formData', 'recordData', 'moduleName'],
  data() {
    return {
      isSaving: false,
    }
  ...
  methods: {
    ...
    afterSerializeHook({ data }) {
      //why this is needed ?.-> copied from facility Bookingsform
      ...
      data.noOfAttendees = data?.internalAttendees?.length || 1
