<template>
  <div class="height100vh">
    <div
      ...
      :class="{
        'header-sidebar-hide': $route.path === 'pdf/prpdf',
      }"
    >
      ...
      <div>
        <div class="print-break-page pr-print-con">
          <prPreview v-if="!isLoading" :details="prDetails"></prPreview>
        </div>
      </div>
    </div>
  </div>
...
<script>
import prPreview from 'src/pages/purchase/pr/PrPreviewParent'
...
export default {
  ...
  computed: {
    id() {
      return this.$route.query.prId ? parseInt(this.$route.query.prId) : ''
    },
  },
  methods: {
    async loadData() {
      this.isLoading = true
      let { purchaserequest, error } = await API.fetchRecord(
        'purchaserequest',
        {
          id: this.id,
        }
      ...
      if (!isEmpty(error)) {
        let {
          message = 'Error Occured while fetching Purchase Request',
        } = error
        this.$message.error(message)
      } else {
        this.prDetails = purchaserequest
