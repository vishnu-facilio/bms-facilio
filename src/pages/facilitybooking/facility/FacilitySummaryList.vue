            ...
            <ul class="sp-ul mT0">
              ...
              >
                ...
                >
                  <el-row>
                    <el-col :span="24">
                      <div class="facility-id">{{ `#${row.localId}` }}</div>
                      <div
  ...
  methods: {
    ...
    switchView(view) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = `/app/bk/facility/${view.name}/${null}/overview`
    ...
    async loadData(loadMore, forceFetch) {
      ...
      if (error) {
        let { message = 'Error Occured while fetching Facility list' } = error
        ...
        if (forceFetch) {
          ...
          if (loadMore) {
            this.facilityList = [...(this.facilityList || []), ...list]
          }
        }
        let listCount = this.$getProperty(meta, 'pagination.totalCount', null)
        this.canLoadMoreList = listCount > (this.facilityList || []).length
      }
      this.loading = false
