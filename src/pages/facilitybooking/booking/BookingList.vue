<template>
  <CommonListLayout
    ...
    pathPrefix="/app/bk/facilitybooking/"
  >
    <template #header>
    ...
    <template #content>
        ...
        >
          ...
          >
            ...
            <div class="q-item-label nowo-label">
              {{ $t('tenant.booking.no_data_booking') }}
            </div>
          </div>
          <template v-else>
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/bk/facilitybooking/viewmanager`"
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Booking'
    },
    isV3Api() {
      return true
    },
    ...
    titleColumn() {
      let { viewColumns } = this
      if (!isEmpty(viewColumns)) {
        return viewColumns.find(column => column.name === 'facility')
      }
      return {}
    },
    moduleName() {
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(
          message || 'Error Occured while fetching booking list'
        )
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`tenant.booking.delete_booking`),
        message: this.$t(`tenant.booking.delete_confirmation_booking`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          let { message } = error
          this.$message.error(message || 'Error Occured while deleting booking')
          this.loading = false
        } else {
          this.$message.success(
            this.$t(`tenant.booking.delete_success_booking`)
          )
