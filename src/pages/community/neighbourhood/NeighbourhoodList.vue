<template>
  <CommonListLayout
    ...
    pathPrefix="/app/cy/neighbourhood/"
  >
    <template #header>
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Neighbourhood'
    },
    appliedFilters() {
      let {
        $route: { query },
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(
          message || 'Error Occured while fetching Neighbourhood list'
        )
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`tenant.neighbourhood.delete`),
        message: this.$t(`tenant.neighbourhood.delete_confirmation`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          let { message } = error
          this.$message.error(
            message || 'Error Occured while deleting neighbourhood'
          )
          this.loading = false
        } else {
          this.$message.success(this.$t(`tenant.neighbourhood.delete_success`))
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-neighbourhood' })
      }
    },
    editRecord({ id = null } = {}) {
      if (!id) return
