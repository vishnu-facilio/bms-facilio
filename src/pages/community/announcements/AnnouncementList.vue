<template>
  <CommonListLayout
    ...
    pathPrefix="/app/cy/announcements/"
  >
    <template #header>
    ...
    <template #content>
            ...
            >
              ...
              >
                <template v-slot="data">
                  ...
                  <template v-else-if="field.name === 'longDescription'">
                    <div class="table-subheading">
                      <div
                        v-html="getDescription(data.row.longDescription)"
                      ></div>
                    </div>
                  </template>
                  <template v-else>
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/cy/announcement/viewmanager`"
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      ...
      return displayName || 'Announcements'
    },
    appliedFilters() {
      let {
        $route: { query },
  ...
  methods: {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`tenant.announcement.delete`),
        message: this.$t(`tenant.announcement.delete_confirmation`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          let { message } = error
          this.$message.error(
            message || 'Error Occured while deleting announcements'
          )
          this.loading = false
        } else {
          this.$message.success(this.$t(`tenant.announcement.delete_success`))
    ...
    addRecord() {
      ...
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-announcement' })
      }
    },
    editRecord({ id = null } = {}) {
      if (!id) return
