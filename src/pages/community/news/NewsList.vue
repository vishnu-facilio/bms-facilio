  ...
  >
    ...
    <template #content>
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/cy/newsandinformation/viewmanager`"
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'News and Information'
    },
    appliedFilters() {
      let {
        $route: { query },
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(message || 'Error occured while fetching news')
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`tenant.news.delete`),
        message: this.$t(`tenant.news.delete_confirmation`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          ...
          this.$message.success(this.$t(`tenant.news.delete_success`))
    ...
    openRecordSummary(id) {
      ...
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'newsSummary', params: { viewname, id } })
      }
    },
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-news' })
      }
    },
    editRecord({ id = null } = {}) {
      if (!id) return
