  ...
  >
    ...
    <template #content>
            ...
            >
              ...
              >
                <template v-slot="data">
                  <template
                    v-if="$getProperty(field, 'field.dataTypeEnum') === 'FILE'"
                  >
                    <FilePreviewColumn
                      :field="field"
                      :record="data.row"
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/sr/admindocuments/viewmanager`"
  ...
  methods: {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`tenant.documents.delete`),
        message: this.$t(`tenant.documents.delete_confirmation`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          ...
          this.$message.success(this.$t(`tenant.documents.delete_success`))
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-admindocs' })
      }
    },
    editRecord({ id = null } = {}) {
      if (!id) return
    ...
    getFileName(field, record) {
      return this.$getProperty(record, `${field.name}FileName`, null)
    },
    openAttachment(field, fileObj) {
      this.selectedFile = this.serializeFile(fileObj)
      this.showPreview = true
    },
    serializeFile(fileObj) {
      let { fileContentType, fileFileName, fileDownloadUrl } = fileObj
      let requiredFileObj = {
        contentType: fileContentType,
        fileName: fileFileName,
        downloadUrl: fileDownloadUrl,
      }
      fileObj = { ...fileObj, ...requiredFileObj }
      return fileObj
    },
  },
}
