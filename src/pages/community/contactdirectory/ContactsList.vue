  ...
  >
    ...
    <template #content>
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/sr/contactdirectory/viewmanager`"
  ...
  methods: {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`tenant.contact_directory.delete`),
        message: this.$t(`tenant.contact_directory.delete_confirmation`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          ...
          this.$message.success(
            this.$t(`tenant.contact_directory.delete_success`)
          )
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-contactdir' })
      }
    },
    editRecord({ id = null } = {}) {
      if (!id) return
