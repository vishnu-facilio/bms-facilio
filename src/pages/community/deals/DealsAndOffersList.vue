  ...
  >
    ...
    <template #content>
            ...
            >
              ...
              >
                <template v-slot="data">
                  <template v-if="field.name === 'attachmentPreview'">
                    <ListAttachments
                      :record="data.row"
                  ...
                  <template v-else-if="field.name === 'description'">
                    <div class="table-subheading">
                      <div v-html="getDescription(data.row.description)"></div>
                    </div>
                  </template>
                  <template v-else>
                    <div
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/cy/dealsandoffers/viewmanager`"
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Deals and Offers'
    },
    appliedFilters() {
      let {
        $route: { query },
    ...
    titleColumn() {
      let { viewColumns } = this
      if (!isEmpty(viewColumns)) {
        return viewColumns.find(column => column.name === 'title')
      }
      return {}
    },
  },
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(
          message || 'Error Occured while fetching Deals and Offers list'
        )
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`tenant.dealsandoffers.delete`),
        message: this.$t(`tenant.dealsandoffers.delete_confirmation`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          ...
          this.$message.success(this.$t(`tenant.dealsandoffers.delete_success`))
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-dealsandoffers' })
      }
    },
    editRecord({ id = null } = {}) {
      if (!id) return
        ...
        if (isWebTabsEnabled()) {
          let { name } =
            findRouteForModule(this.moduleName, pageTypes.EDIT) || {}
    ...
    getDescription(description) {
      return !isEmpty(description) ? htmlToText(description) : '---'
    },
  },
}
</script>
