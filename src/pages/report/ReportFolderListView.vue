...
import PivotFolderRename from 'pages/energy/pivot/Components/PivotFolderRename'
import PivotReportShare from 'pages/energy/pivot/Components/PivotReportShare'
...
import { isWebTabsEnabled, pageTypes, findRouteForTab, findRouteForReport } from '@facilio/router'
export default {
  props: ['moduleName','tabId'],
  mixins: [ReportHelper,ModularAnalyticmixin, ReportTabPermissions],
  components: {
    draggable,
    PivotFolderRename,
    PivotReportShare,
  ...
  data() {
    return {
      isLoading: false,
      reportFolders: [],
      url: '/v3/report/folders?moduleName=',
      editFolder: null,
  ...
  methods: {
    ...
    async loadFoldersList(addFolder) {
      ...
      if (!error) {
        ...
        this.$message.error('Error on loading report folders')
      }
    },
    ...
    async editReport(report) {
      ...
      if (moduleName === 'energydata' || (isWebTabsEnabled() && this.moduleName === 'energydata')) {
        let resultObj
        let routePath
        let {
          ANALYTIC_PORTFOLIO,
        ...
        await API.put(url).then(response => {
          if (!response.error) {
        ...
        if (isWebTabsEnabled()) {
          moduleName = this.$attrs.moduleName ? this.$attrs.moduleName : this.moduleName
