<template>
  <div
    class="reports-summary new-reports-summary"
  ...
  >
    ...
    >
      ...
      >
        <el-breadcrumb class="pdf-report" separator="">
          <el-breadcrumb-item
            v-if="resultObj"
            class="pivot-crumb description"
            style="font-weight:500 !important"
            v-model="reportObj.report"
          >
            <div v-if="resultObj" class="breadcrumb-text-folder">
              {{
                resultObj.report.description != null
                  ? resultObj.report.description
                  : 'No Description'
              }}
            </div>
          </el-breadcrumb-item>
          ...
          <el-breadcrumb-item
            class="pivot-crumb report-name"
            style="font-weight:500 !important;font-size:16px !important;"
            v-if="reportObj"
          >
            {{ resultObj.report.name }}</el-breadcrumb-item
          >
        </el-breadcrumb>
        <div class="reading-report-options">
          <f-report-options
            optionClass="report-options"
            :optionsToEnable="[1, 2, 4]"
            :params="reportParams"
        ...
        <div class="fc-logo-report" v-if="showPrintDetails && $org.logoUrl">
          <img :src="$org.logoUrl" class="logo-pdf" />
        </div>
        <div class="fc-logo-report" v-else-if="showPrintDetails">
          <img src="~assets/facilio-logo-black.svg" />
        </div>
      </div>
    </div>
...
.new-reports-summary .fc-new-chart svg {
  background: #f7f8f9;
}
.new-reports-summary .full-layout-white {
  background: #f7f8f9 !important;
}
.new-reports-summary .tabular-report-table th {
  background: #f7f8f9 !important;
}
.new-reports-summary .widget-legends {
  background: #f7f8f9;
  margin-left: 100px !important;
}
.new-reports-summary .report-userfilter-select .el-input .el-input__inner {
  background: #f7f8f9 !important;
  border: solid 1px #d0d9e2;
}
.new-reports-summary .fwidget-report-period-select input {
  background: #f7f8f9 !important;
}
.new-reports-summary .f-multichart .bb-zoom-rect {
  fill: #f7f8f9;
}
...
@media print {
  ...
  .new-reports-summary .fc-new-chart {
    page-break-after: avoid !important;
  }
  .new-reports-summary .tabular-table-widget {
  ...
  .el-breadcrumb.pdf-report {
    margin: auto auto !important;
  }
  span.el-breadcrumb__item.pivot-crumb.description {
    display: none !important;
  }
  span.el-breadcrumb__item.pivot-crumb.report-name {
    justify-content: center !important;
    align-items: center !important;
  }
  .report-layout-pivot .header-bar-background {
