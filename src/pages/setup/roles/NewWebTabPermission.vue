        ...
        <div v-if="showEmptyState" class="empty-state-class">
          ...
          <template v-if="selectedLayout !== layoutTypes.DEFAULT_VIEW">
            <div v-if="$validation.isEmpty(groupPermissions)">
              {{
                $t('common._common.no_groups_available_app', {
                  appName: appName,
                })
              }}
            </div>
            ...
            >
              {{ $t('common._common.no_tabs_associated_for_group') }}
            </div>
          </template>

          <div v-else>
            {{
              $t('common._common.no_tabs_available_app', {
                appName: appName,
              })
            }}
          </div>
                  ...
                  >
                    ...
                    <div class="mL20">
                      <el-radio-group
                        ...
                        "
                        v-model="permissionVal.actionName"
                        class="fc-radio-btn"
                      >
                        <el-radio
                          v-for="(perm, permName) in permissionVal.permissions"
                          :key="perm.actionName"
                          :label="perm.actionName"
                        >
                          {{ permName }}
                        </el-radio>
                      </el-radio-group>
...
export default {
  ...
  methods: {
    async loadTabs() {
      let { role } = this
      let { roleId } = role || {}
      let additionalParams = { roleId }
      let { error, data } = await loadAppTabs(this.appId, additionalParams)
    ...
    async loadAppLayouts() {
      ...
      let { roleId } = role || {}
      let additionalParams = { roleId }
      let { error, data } = await loadLayouts(
        this.appId,
        true,
        additionalParams
      )
    ...
    constructTabPermission(tabsList) {
      let tabPermissions = tabsList.reduce((tabWithPermission, tab) => {
        let { permission, id, name } = tab
        let tabPermissions = this.setTabPermissions(permission)
        let { tabHasPermission, permissionList } = tabPermissions || {}
        let tabPermission = {
          id,
          name,
          tabPermission: tabHasPermission,
          permission: permissionList,
        }
        tabWithPermission.push(tabPermission)
        return tabWithPermission
      }, [])
      this.tabPermissions = tabPermissions.filter(
        tab => !isEmpty(tab.permission) || !isEmpty(tab.permission2)
      )
    },
    setTabPermissions(permission) {
      let tabHasPermission = false
      let permissionList = []
      if (!isEmpty(permission)) {
        permissionList = permission.reduce((permissionObj, permName) => {
          let {
            actionName,
            displayName,
            value,
          ...
          if (parentPermName) {
            ...
            if (!isEmpty(permissions)) {
              ...
              let rolePermission = permissions.reduce((rolePerm, perm) => {
                ...
                if (enabled) {
                  permissionEnabled = { enabled, value, permissionMapping }
                  tabHasPermission = true
      ...
      return { permissionList, tabHasPermission }
    },
    constructGroupPermission() {
      ...
      if (!isEmpty(groups)) {
        let groupPermissions = groups.reduce((groupWithPermission, group) => {
          ...
          webTabs = webTabs.filter(tab => !isEmpty(tab.permission))
          ...
          if (!isEmpty(tabs) && Object.keys(tabs).length > 0) {
    ...
    togglePermission(tab, permissionName) {
      ...
      let actualPermission = (permission || {})[permissionName]
      if (!isEmpty(permissions)) {
        if (enabled) {
          let key = Object.keys(permissions)[0]
          let { value, permissionMapping, actionName } = (permissions || {})[
            key
          ]
          if (!isEmpty(actualPermission)) {
            this.$setProperty(tab, `permission.${permissionName}.value`, value)
            this.$setProperty(
              tab,
              `permission.${permissionName}.permissionMapping`,
              permissionMapping
            )
            this.$setProperty(
              tab,
              `permission.${permissionName}.actionName`,
              actionName
            )
          }
        } //First role
          ...
          if (!isEmpty(actualPermission)) {
            this.$setProperty(tab, `permission.${permissionName}.value`, 0)
          }
        }
      }
      if (!isEmpty(subPermission)) {
        Object.keys(subPermission).forEach(perm => {
          let { permission } = tab || {}
          let permissionObj = (permission || {})[permissionName]
          let { subPermission } = permissionObj || {}
          let actualSubPermission = (subPermission || {})[perm]
          if (!isEmpty(actualSubPermission)) {
            let subPermission =
              this.$getProperty(
                tab,
                `permission.${permissionName}.subPermission`
              ) || {}

            this.$setProperty(subPermission, `${perm}.enabled`, enabled)
            this.$setProperty(
              tab,
              `permission.${permissionName}.subPermission`,
              subPermission
      ...
      if (!isEmpty(permission)) {
        Object.values(permission).forEach(tabPerm => {
          ...
          if (!isEmpty(permissions)) {
            if (enabledVal) {
              let key = Object.keys(permissions)[0]
              let actualPermission = permissions[key] || {}
              let { value, permissionMapping, actionName } = actualPermission
              if (!isEmpty(actualPermission)) {
                tabPerm.value = value
                tabPerm.permissionMapping = permissionMapping
                tabPerm.actionName = actionName
              }
            } else {
              tabPerm.value = 0
            }
            if (!isEmpty(subPermission)) {
              Object.keys(subPermission).forEach(perm => {
                let { subPermission } = tabPerm || {}
                let actualSubPerm = (subPermission || {})[perm]
                if (!isEmpty(actualSubPerm)) {
                  this.$setProperty(
                    tabPerm,
                    `subPermission.${perm}.enabled`,
                    enabledVal
                  )
                }
              })
    ...
    serialize() {
      let permissions = this.tabPermissions
        .map(tab => {
          let { id, permission } = tab
          if (permission) {
            let tabObj = {
              id,
              permission: this.getAllPermissionForTab(permission),
            }
            return tabObj
          } else {
            return null
    ...
    getAllPermissionForTab(permission) {
      let permissionList = []
      Object.values(permission).map(value => {
        if (!isEmpty(value['subPermission'])) {
          permissionList.push(...Object.values(value['subPermission']))
        }
