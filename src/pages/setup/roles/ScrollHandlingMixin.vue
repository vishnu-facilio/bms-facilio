...
export default {
  data() {
    return {
      rootElementForScroll: '.tab-content-layout',
      sidebarElements: [],
      sectionElements: [],
    }
  },
  ...
  methods: {
    setElementsId() {
      this.tabListForGroup.forEach(tab => {
        let { name } = tab

        this.sidebarElements.push(`#${this.trimTabName(name)}-link`)
        this.sectionElements.push(`#${this.trimTabName(name)}-section`)
      })
    },

    trimTabName(tabName) {
      let trimmedName = tabName.replace(/\W+/g, '')
      return trimmedName
    },
    ...
    scrollhandler() {
      let { highlightSidebarElement, layoutType, layoutTypes } = this
      ...
      window.requestAnimationFrame(() => {
        elements.forEach(el => {
          ...
          let topScrollHeight =
            layoutType !== layoutTypes.DEFAULT_VIEW ? 200 : 160

          if (top <= topScrollHeight) {
            highlightSidebarElement(sectionName)
          }
        })
      })
