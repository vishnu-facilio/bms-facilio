<template>
  <div class="fc-user-scope-criteria">
      ...
      <div class="criteria-inner-container">
        ...
        >
          ...
          <div class="creteria-dropdown">
            ...
            >
              <el-option
                v-for="(operator, indexOpertator) in getOperators(criteria)"
                :key="indexOpertator"
                :label="operator.operator"
                :value="operator.operator"
                :selected="indexOpertator === 0"
              ></el-option>
            </el-select>
          </div>
          ...
          <div>
            <el-select
              ...
              collapse-tags
              filterable
              v-model="fieldValues[criteria.fieldName]"
              class="select-operator-input fc-border-select"
              :default-first-option="true"
            >
              <el-option
          ...
          <!-- <div v-if="$validation.isEmpty(fieldValues[criteria.fieldName])">
            <el-input
              placeholder="Input not needed"
              disabled
              class="fc-border-select pL10"
            ></el-input>
          </div> -->

          <div class="pL10" v-if="fieldValues[criteria.fieldName] === '93'">
            <el-select
              filterable
            ...
            >
              <el-option
                v-for="(generator, valueInex) in valueGeneratorData"
                :key="valueInex"
                :value="generator.linkName"
                :label="generator.valueGeneratorName"
              >
              </el-option>
            </el-select>
          </div>
          ...
          <div
            ...
            "
          >
            <div v-if="criteria.operatorsDataType">
              <div
                v-if="
                  !$validation.isEmpty(lookupModuleFieldsList) &&
              ...
              >
                <el-select
                  ...
                  filterable
                  v-model="criteria.value"
                  class="fc-select-multiple-tag fc-criteria-value-select fc-tag width100 select-operator-input fc-border-select"
                >
                  <el-option
...
export default {
  ...
  async mounted() {
    this.addCreteria()
    await this.loadWoForms()
    await this.loadModuleMeta(this.exrule)
    this.lookupFieldHandling()
    await this.getValueGeneratorData()
    if (
      this.exrule?.conditions[1] &&
      this.exrule.conditions[1].operatorId &&
      this.exrule.conditions[1].operatorId === 93
    ) {
      this.$set(this.exrule['conditions'][1], 'valueTypes', 93)

      this.$set(this.exrule['conditions'][1], 'operatorsDataType', {
        dataType: 'LOOKUP',
      ...
      if (this.exrule['conditions'][1].columnName === 'SITE_ID') {
        this.$set(
          this.exrule['conditions'][1],
          'columnName',
          'WorkOrders.SITE_ID'
      ...
      this.$set(this.exrule['conditions'][1], 'operatorLabel', 'is')
      this.$set(this.fieldValues, this.exrule['conditions'][1].fieldName, '93')
      this.chageDataType(this.exrule)
    }
  },
  methods: {
    lookupFieldHandling() {
    ...
    async loadModuleMeta(editCondi) {
      this.loading = true
    ...
    getOperators(criteria) {
      ...
      if (criteria.fieldName && criteria.fieldName !== '') {
        ...
        let { dataTypeEnum, name: fieldName } = field || {}
        let { _name } = dataTypeEnum || {}
        if (fieldName === 'siteId') {
          _name = 'LOOKUP'
        }
        ...
        if (_name) {
          let oprs = this.moduleMeta.operators[_name] || {}
          // let operatorObject = Object.values(oprs).find(
          //   opr => opr.defaultSelection
          // )
          return Object.values(oprs).filter(opr => opr.defaultSelection)
    ...
    async getValueGeneratorData() {
      this.loading = true
      let { error, data } = await API.get('v2/scoping/valueGenerators')
      if (error) {
        this.$message.error(error.message || 'Error Occurred')
      } else {
        this.valueGeneratorData = data.valueGenerators
      }
      this.loading = false
    },
  },
