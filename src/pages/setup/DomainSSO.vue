...
export default {
  ...
  methods: {
    ...
    downloadMetaUrl() {
      return '/api/v2/setup/portalsso/metadata?appDomainType=serviceportal'
    },
    getUpdateUrl() {
      return '/v2/setup/portalsso/update?appDomainType=serviceportal'
    },
    ...
    updateCreateUserStatus() {
      ...
      this.$http
        .post(
          '/v2/setup/portalsso/updateCreateUserStatus?appDomainType=serviceportal',
          ssoData
        )
        .then(response => {
          let { status } = response
