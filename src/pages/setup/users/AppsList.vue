  ...
  <div class="user-myapp-list">
    <div>
      ...
      >
        ...
        <el-table-column width="300" prop="description" label="Description">
          <template v-slot="myApps">
            {{ $getProperty(myApps, 'row.application.description', '---') }}
          </template>
        </el-table-column>
        <el-table-column width="200" label="Role" prop="roleName">
          <template v-slot="myApps">
            {{ $getProperty(myApps, 'row.role.name', '---') }}
          </template>
        </el-table-column>
        <el-table-column width="260" prop="isDefaultApp">
          <template v-slot="myApps">
            ...
            <!-- {{
              $getProperty(myApps, 'row.isDefaultApp') == true ? 'Yes' : 'No'
            }} -->
          </template>
        </el-table-column>
...
import MoreOptionPopover from './MoreOptionPopover.vue'
...
export default {
  ...
  methods: {
    ...
    async listMyAppsForUser() {
      this.loading = true
      let { summaryId, applicationId } = this
      let { error, data } = await API.get(`v3/myApps/listMyAppsForUser`, {
        ouId: summaryId,
        appId: applicationId,
      },{ cacheTimeout : 0 })
      if (error) {
