    ...
    <template>
      ...
      <div class="fc-show-filter-con" v-if="showFilter">
        <div class="flex-middle">
          ...
          <div>
            <div class="fc-black-13 text-left bold pB5">Type</div>
            <el-select
              placeholder="Select"
      ...
      <div class="occupantPortal-tabrow pL0 pR0">
        ...
        <setup-empty
          v-else-if="$validation.isEmpty(occupantUsers) && !loading"
          ...
          <table class="setting-list-view-table" width="100%">
            ...
            <tbody>
              <tr
                ...
                v-for="(user, index) in occupantUsers"
                :key="index"
              >
                <td>
...
export default {
  ...
  methods: {
    ...
    async loadOccupantsCount() {
      ...
      } else if (this.activeTab === 'Occupants') {
        filters = {
          isOccupantPortalAccess: {
            value: ['true'],
            operatorId: 15,
      ...
      } else if (this.activeTab === 'tenantContacts') {
        filters = {
          isOccupantPortalAccess: {
            value: ['true'],
            operatorId: 15,
      ...
      if (error) {
        ...
        this.occupantUsersCount = data.recordCount || []
      }
      this.loading = false
    },
    async loadOccupants() {
      ...
      if (error) {
        ...
        this.occupantUsers = data.people || []
      }
      this.loading = false
    },
    changeActiveTab() {
      this.loadOccupants()
      this.loadOccupantsCount()
    },
    ...
    async removeOccupant(user) {
      let url = ''
      let successMsg = ''
      url = '/v2/people/update'
      successMsg = this.$t('common._common.delete_success')
      user.rolesMap['service'] = null
      user.isOccupantPortalAccess = false
