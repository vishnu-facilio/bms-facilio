<template>
  <div class="height100vh">
    ...
    <!-- container -->
    <f-dialog
      v-if="showClientOptionDialog"
      :visible.sync="showClientOptionDialog"
      :width="'40%'"
      maxHeight="350px"
      :title="$t('common._common.remove_access')"
      @save="portalAccessRemove(currentUser)"
      @close=";(showClientOptionDialog = false), (selectedDelete = false)"
    ...
    >
      <div class="height150">
        <div class="label-txt-black text-left pT10">
          {{
            $t(
              'common.header.are_you_sure_you_want_to_remove_client_portal_access',
...
import ClientUserForm from 'pages/setup/portal/PortalUserForm'
...
export default {
  ...
  methods: {
    ...
    async loadclientUser() {
      ...
      filters.isClientPortalAccess = {
        operatorId: 15,
        value: ['true'],
      }
      await API.get(
        `/v2/clientcontact/list?page=${this.page}&perPage=${this.perPage}`,
        {
        ...
        if (error) {
          ...
          this.userlist = data.clientcontacts || []
        }
        this.loading = false
      })
    },
    async loadClientUserCount() {
      this.loading = true
      let filters = {}
      let genericSearch
      if (this.querySearch) {
      ...
      await API.get('/v2/clientcontact/list?fetchCount=true', {
        filters: !isEmpty(filters) ? JSON.stringify(filters) : null,
        genericSearch,
      }).then(({ data, error }) => {
        if (error) {
    ...
    showDelete(user) {
      this.currentUser = user
      this.showClientOptionDialog = true
    },
    getRoleName(user) {
    ...
    saveClient(client) {
      ...
      let successMsg = ''
      if (isNew) {
        url = '/v2/clientcontact/add'
        successMsg = 'New client Added.'
      } else {
        url = '/v2/clientcontact/update'
      ...
      let params = {
        clientContacts: [
          { ...pick(client, props), peopleType: 4, isClientPortalAccess: true },
        ],
      }
    ...
    portalAccessRemove(user) {
      ...
      if (!selectedDelete) {
        url = '/v2/clientcontact/updateClientAppAccess'
        params = {
          clientContacts: [{ id, isClientPortalAccess: false }],
        }
        successMsg = 'Access Removed successfully'
      } else {
        url = '/v2/clientcontact/delete'
        params = {
          clientContactIds: [id],
        }
      ...
      API.post(url, params).then(({ error }) => {
        ...
        this.showClientOptionDialog = false
      })
    },
    updateConfig(config) {
      this.app = {
