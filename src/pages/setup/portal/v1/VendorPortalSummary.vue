<template>
  <div class="height100vh">
        ...
        <div>
          ...
          <div class="setting-title-block flex-middle justify-content-space">
            <div class="flex-middle pT10">
              <div
                class="fc-portal-icon fc-portal-icon-vendor-bg flex-center-vH align-center"
              >
                <inline-svg
    ...
    <div class="fc-main d-flex relative">
      ...
      <!-- <div
        @click="showHideArrow"
        class="fc-close-arrow pointer"
        v-if="showSidebar"
        v-tippy="{ arrow: true, arrowType: 'round' }"
        content="Close Sidebar"
      >
        <i class="el-icon-d-arrow-left f18"></i>
      </div> -->
            ...
            >
              ...
              <div class="flex-middle">
                ...
                >
                  ...
                  ></inline-svg>
                  <div :class="{ 'dot-active-pink': querySearch }"></div>
                </div>
              </div>
            </div>
    ...
    <!-- container -->
    <f-dialog
      v-if="showVendorOptionDialog"
      :visible.sync="showVendorOptionDialog"
      :width="'40%'"
      maxHeight="350px"
      :title="$t('common._common.remove_access')"
      @save="portalAccessRemove(currentUser)"
      @close="showVendorOptionDialog = false"
    ...
    >
      <div class="height150">
        <div class="label-txt-black text-left pT10">
          {{
            $t(
              'common.header.are_you_sure_you_want_to_remove_vendor_portal_access',
              { name: currentUser.name }
            )
          }}?
        </div>
        <div class="label-txt-black text-left pT20">
          <el-checkbox v-model="selectedDelete" class="label-txt-black fw4">
            {{
...
import VendorUserForm from 'pages/setup/portal/PortalUserForm'
...
export default {
  ...
  data() {
    return {
      ...
      fetchParams: {
        ...
        url: 'v2/vendorcontact/list',
        dataKey: 'vendorcontacts',
      },
  ...
  methods: {
    ...
    async loadvendorUser() {
      ...
      filters.isVendorPortalAccess = {
        operatorId: 15,
        value: ['true'],
      }
      await API.get(
        `/v2/vendorcontact/list?page=${this.page}&perPage=${this.perPage}`,
        {
        ...
        if (error) {
          ...
          this.userlist = data.vendorcontacts || []
        }
        this.loading = false
      })
    },
    async loadVendorUserCount() {
      this.loading = true
      let filters = {}
      let genericSearch
      if (this.querySearch) {
      ...
      await API.get('/v2/vendorcontact/list?fetchCount=true', {
        filters: !isEmpty(filters) ? JSON.stringify(filters) : null,
        genericSearch,
      }).then(({ data, error }) => {
        if (error) {
    ...
    showDelete(user) {
      this.currentUser = user
      this.showVendorOptionDialog = true
    },
    getRoleName(user) {
    ...
    saveVendor(vendor) {
      ...
      let successMsg = ''
      if (isNew) {
        url = '/v2/vendorcontact/add'
        successMsg = 'New Vendor Added.'
      } else {
        url = '/v2/vendorcontact/update'
        successMsg = 'Vendor Updated.'
      }
      ...
      let params = {
        vendorContacts: [
          { ...pick(vendor, props), peopleType: 2, isVendorPortalAccess: true },
        ],
      }
    ...
    portalAccessRemove(user) {
      ...
      if (!selectedDelete) {
        url = '/v2/vendorcontact/updatePortalAccess'
        params = {
          vendorContacts: [{ id, isVendorPortalAccess: false }],
        }
        successMsg = 'Access Removed successfully'
      } else {
        url = '/v2/vendorcontact/delete'
        params = {
          vendorContactIds: [id],
        }
      ...
      API.post(url, params).then(({ error }) => {
        ...
        this.showVendorOptionDialog = false
      })
    },
    updateConfig(config) {
      this.app = {
