                    ...
                    <tbody v-else-if="userlist.length === 0">
                      <tr>
                        <td colspan="100%" class="text-center">
                          {{
                            $t(
                              'common.products.no_vendor_portal_users_available'
                            )
                          }}
                        </td>
                      </tr>
...
import VendorUserForm from './PortalUserForm'
...
export default {
  ...
  methods: {
    ...
    loadVendorUserCount() {
      ...
      API.get('/v2/vendorcontact/list?fetchCount=true', params).then(
        ({ data, error }) => {
          if (error) {
            this.$message.error(error.message || 'Error Occured')
          } else {
    ...
    saveVendor(vendor) {
      ...
      let props = ['name', 'email', 'phone', 'vendor', 'rolesMap', 'language']
