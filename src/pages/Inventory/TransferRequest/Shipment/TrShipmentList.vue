          ...
          <template v-slot="{ record }">
            ...
            >
              <div
                ...
                :title="
                  $getProperty(record, 'transferRequest.requestSubject') ||
                    '---'
                "
                v-tippy="{
                  placement: 'top',
          ...
          >
            ...
            <template #[slotList[1].criteria]="{record}">
              ...
              >
                <el-tooltip
                  ...
                  :open-delay="600"
                >
                  <div class="self-center truncate-text">
            ...
            <template #[slotList[2].name]="{record}">
              <div class="d-flex text-center">
                <i
                  v-if="checkPermission(record, 'UPDATE')"
...
export default {
  name: 'TransferRequestShipmentList',
  ...
  computed: {
    slotList() {
      return [
        ...
        {
          criteria: JSON.stringify({ name: 'transferRequest' }),
        },
        {
          name: 'editDelete',
          isActionColumn: true,
    ...
    parentPath() {
      return `/app/inventory/trShipment`
    },
  },
  methods: {
    checkPermission(record, action) {
      let { isCompleted } = record || {}
      return this.hasPermission(action) && !isCompleted && record.canEdit()
    },
    ...
    hasPermission(action) {
      return this.$hasPermission(`transferrequest:${action}`)
    },
    openList() {
