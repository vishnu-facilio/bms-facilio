  ...
  computed: {
    slotList() {
      return [
        ...
        {
          criteria: JSON.stringify({ name: 'requestSubject' }),
        },
        {
          name: 'editDelete',
          isActionColumn: true,
    ...
    parentPath() {
      return `/app/inventory/transferrequest`
    },
  },
  methods: {
    checkPermission(record, action) {
      let { isStaged } = record || {}
      return this.hasPermission(action) && !isStaged && record.canEdit()
    },
    ...
    redirectToFormCreation() {
      ...
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-transferrequest' })
      }
    },
  },
}
