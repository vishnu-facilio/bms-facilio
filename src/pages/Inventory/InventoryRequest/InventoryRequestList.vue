          ...
          >
            ...
            <template #[slotList[2].name]="{record}">
              <div class="d-flex text-center">
                <i
                  v-if="
                    hasPermission('EDIT,UPDATE') &&
                      record.canEdit() &&
                      isReservationStatusPending(record)
                  "
...
export default {
  ...
  computed: {
    parentPath() {
      let { moduleName } = this
      return `/app/inventory/${moduleName}/`
    },
  },
  methods: {
    isReservationStatusPending(record) {
      let { inventoryRequestReservationStatusEnum } = record
      let isPending =
        inventoryRequestReservationStatusEnum === 'PENDING' ||
        isEmpty(inventoryRequestReservationStatusEnum)
      return isPending
    },
    editModule(row) {
      let { moduleName } = this
