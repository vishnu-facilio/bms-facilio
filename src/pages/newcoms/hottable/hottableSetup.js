...
export default {
  methods: {
    /* Initializing data related methods */
    setupHotTableSettingsData() {
      ...
      */
      ...
      hotTableSettingsData.beforeColumnSort = function(
        currentSortConfig,
        destinationSortConfigs
      ) {
        const columnSortPlugin = this.getPlugin('columnSorting')
        columnSortPlugin.setSortConfig(destinationSortConfigs)
        let { handleSorting } = compInstance || {}
        if (isFunction(handleSorting)) {
          return compInstance.handleSorting(
            currentSortConfig,
            destinationSortConfigs,
            columnSortPlugin,
            compInstance
      ...
      hotTableSettingsData.afterChange = function(changes, source) {
        ...
        if (!isEmpty(changes) && source !== 'CopyPaste.paste') {
          ...
          for (let [, rowData] of changes.entries()) {
            let [row, prop, oldValue, newValue] = rowData
            let { [row]: rowValue } = changedValues || {}
            if (isEmpty(rowValue)) {
              rowValue = [rowData]
            } else {
              rowValue.push(rowData)
            }
            changedValues[row] = rowValue
          ...
          if (source === 'Autofill.fill') {
            ...
            for (let item of Object.keys(changedValues)) {
              promises.push(
                compInstance.onAutoFillRowChange({
                  index: item,
                  rowChanges: changedValues[item],
                })
              )
            }
          }
