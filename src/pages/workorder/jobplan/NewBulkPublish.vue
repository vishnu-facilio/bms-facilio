<template>
  ...
  >
    <div class="height180 d-flex">
      <i
        class="fa fa-exclamation-triangle pR5 pL10 bulk-update-icon"
        aria-hidden="true"
      ></i>
      <div class="d-flex flex-direction-column mL10 width100">
        ...
        <div class="dialog-content mT15" v-if="stateUpdating">
          <spinner :show="true" size="80"></spinner>
        </div>
        <div v-else-if="showStatus && statusMessage" class="status-message">
          {{ statusMessage }}
        </div>
        <div v-else v-html="dialogContent" class="dialog-content mT15"></div>
      </div>
    </div>
    <div class="modal-dialog-footer">
      <template v-if="showStatus">
        <el-button @click="closeDialog" class="modal-btn-save full-width">{{
          `CLOSE`
        }}</el-button>
      </template>
      <template v-else>
...
export default {
  ...
  computed: {
    btnMessage() {
      return this.stateUpdating ? 'PUBLISHING...' : this.buttonText
    },
    dialogHeader() {
