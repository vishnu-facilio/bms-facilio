...
export default {
  ...
  methods: {
    ...
    handleDelete(workorderID) {
      console.log('deleting ', workorderID)
    ...
    handleDrop(e) {
      ...
      this.$http.post(url, reqJson).then(resp => {
        this.requestPending = false
        console.log('update PM WO response')
      })
    },
    handleStaffChange(event, newAssignToID) {
      ...
      this.requestPending = true
      console.log('reassign to request', newAssignToID)
      let url = '/v2/pmplanner/jobs/update'
    ...
    handleViewChange(e) {
      console.log('fetching date for range', e)
      this.currentRange = e
