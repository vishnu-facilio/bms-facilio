                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <el-option
                            :label="categoryOptionsForTasks[0].label"
                            :value="{
                              index: key,
                              taskIndex: taskKey,
                              categ: [categoryOptionsForTasks[0].value],
                            }"
                            class="visibility-visible-actions pm-task-active-remove"
                          >
                            <div class="fL width200px textoverflow-ellipsis">
                              {{ categoryOptionsForTasks[0].label }}
                            </div>
                            <div
                              v-if="model.woData.workOrderType !== 'single'"
                              class="fR visibility-hide-actions"
                              @click.stop="
                                setCurrentPosition(key, taskKey),
                                  handleTaskSearchSelect(key, taskKey, [
                                    categoryOptionsForTasks[0].value,
                                  ])
                              "
                            >
                              <img
        ...
        >
          ...
          >
            <el-col :span="10">
              <div>
                <el-input
                  type="text"
                  v-model="option.name"
                  @change="setDefaultValue(taskSettingsEdit)"
            ...
            <el-col v-if="taskSettingsEdit.inputType === '5'" :span="2">
              <div
                @click="
                  removeOption(taskSettingsEdit.options, index)
                  setDefaultValue(taskSettingsEdit)
                "
        ...
        >
          ...
          >
            <el-row class="visibility-visible-actions">
              ...
              >
                <div
                  @click="removeOption(sectionSettingsEdit.options, index)"
  ...
  computed: {
    ...
    categoryOptionsForTasks() {
      ...
      if (category[0] === 'CURRENT_FLOOR') {
        ...
        this.$http
          .get(
            `/workorder/getScopeFilteredValuesForPM?${buildingId}siteId=${
              this.model.woData.woModel.site.id
            }&spaceCategoryId=${Number(
              category[1]
            )}${parentResourceIds}&parentAssignmentType=${
              this.model.woData.resourceType === 'ALL_FLOORS' ? 1 : 3
            }&assignmentType=3${includeIds}`
          )
          .then(response => {
    ...
    filter() {
      ...
      if (this.model.woData.workOrderType === 'single') {
        if (
          ...
          if (this.model.woData.singleResource.buildingId > 0) {
            filter.buildingId = this.model.woData.singleResource.buildingId
          }

          if (this.model.woData.singleResource.spaceId > 0) {
            filter.spaceId = this.model.woData.singleResource.spaceId
          }

          if (this.model.woData.singleResource.floorId > 0) {
            filter.floorId = this.model.woData.singleResource.floorId
          }
        }
      } else {
        if (
  ...
  methods: {
    ...
    handleFloorCategoryOptions() {
      ...
      if (this.categoryOptions.length < 3) {
        ...
        this.$http
          .get(
            `/workorder/getScopeFilteredValuesForPM?${buildingId}siteId=${this.model.woData.woModel.site.id}&assignmentType=1${includeIds}`
          )
          .then(response => {
    ...
    handleSpaceCategoryOptions(isSingleWo) {
      ...
      if (this.categoryOptions.length < 3) {
        ...
        if (isSingleWo) {
          ...
          url = `/workorder/getScopeFilteredValuesForPM?${buildingId}siteId=${this.model.woData.woModel.site.id}&spaceCategoryId=${this.model.woData.spacecategoryId}&assignmentType=3${includeIds}`
        }
