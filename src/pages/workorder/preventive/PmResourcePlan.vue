        ...
        <div class="fc-pm-main-bg fc-pm-main-bg2">
          <div class="fc-pm-main-content">
            <div>
              ...
              <div class="fR fc-black-small-txt-12">
                <div class="flex-middle">
                  <div class="">
                    ...
                    <span>{{ to }}</span>
                    <template>
                      of {{ model.woData.resourceList.length }}
                      <span
                      ...
                      <span
                        ...
                        v-bind:class="{
                          disable: to === model.woData.resourceList.length,
                        }"
                      ></span>
                    </template>
                  </div>
              ...
              <!-- bulk selection menu,  comes only when select all is checked -->
              <div
                v-show="
                  selectAll ||
                ...
                <table class="fc-pm-form-table">
                  ...
                  <tbody>
                    <tr
                      ...
                      v-if="isnormalizedSearchQuery(key)"
                      :key="key"
                      class="trigger-tr-visibility visibility-visible-actions"
                      v-show="
                        !normalizedSearchQuery ||
                          resource.normalizedName.includes(
                            normalizedSearchQuery
                          )
                      "
                    >
                      ...
                      <!-- resource trigger column -->
                      <td style="width: 250px;" class="text-left">
                        <el-select
                          @change="
                            val => {
                              let zi = model.woData.resourceList[
                                key
                              ].triggerNames.findIndex(i => i === '0')
                              let di = model.woData.resourceList[
                                key
                              ].triggerNames.findIndex(
                                i => i === model.triggerData.triggers[0].name
                              )
                              if (di >= 0 && zi >= 0) {
                                model.woData.resourceList[
                                  key
                                ].triggerNames.splice(zi, 1)
                              }
                              forceUpdate()
                            }
                          "
...
export default {
  ...
  data() {
    return {
      searchTerm: '', // rp uses this for filtering
  ...
  methods: {
    isnormalizedSearchQuery(key) {
      return (
        this.normalizedSearchQuery || (key >= this.from - 1 && key < this.to)
      )
    },
    filterResults() {
    ...
    selected(val) {
      for (let i = 0; i < this.model.woData.resourceList.length; i++) {
        if (
          ...
          this.model.woData.resourceList[i].normalizedName.includes(
    ...
    pageCalculation() {
      ...
      let resourceListLength = this.$getProperty(
        this,
        'model.woData.resourceList.length'
      )
      if (this.to >= resourceListLength) {
        this.to = resourceListLength
        this.hasMorePage = false
      } else {
        this.hasMorePage = true
      }
