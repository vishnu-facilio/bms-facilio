          ...
          <div class="fc-pm-main-inner-container">
            <div class="fc-pm-task-form-table-con">
              <div class="review-wrap-block">
                <el-row class="pB15">
                  <el-col :span="12">
                    <div class="fc-red-txt14 pB5">Site</div>
                ...
                <el-row class="pB15 pT15">
                  <el-col :span="12">
                    <div class="pB5 fc-summary-content-header">PM Type</div>
                    <div
...
export default {
  ...
  methods: {
    ...
    validated() {
      let isMultiSitePMLicenseEnabled = this.$helpers.isLicenseEnabled(
        'MULTISITEPM'
      )
      if (isMultiSitePMLicenseEnabled) {
        this.saveAndMigrateSingleSitePMToMultiSitePM()
      } else {
        this.save()
      }
    },
    ...
    save() {
      ...
      let siteId = this.$getProperty(this, 'model.woData.woModel.site.id', -1)
      if (siteId <= 0) {
        this.$message({
      ...
      for (let key in this.model.taskData.taskSections) {
        ...
        for (let key2 in section.taskTemplates) {
          ...
          task.siteId =
            this.model.woData.woModel.site && this.model.woData.woModel.site.id
    ...
    async saveAndMigrateSingleSitePMToMultiSitePM() {
      this.isSaving = true
      this.model.emitForm = false
      let siteId = this.$getProperty(this, 'model.woData.woModel.site.id', -1)
      if (siteId <= 0) {
      ...
      if (
        ...
        (workorder.resource && !workorder.resource.id)
      ) {
        delete workorder.resource
      } else if (workorder.resource && workorder.resource.id > 0) {
        workorder.resource.triggerNames = ['0']
        workorder.resource.notifications = []
        // TODO: Set RESOURCE_TYPE
        // update this.model.woData.resourceType for setting assignmentType
        if (
          this.$getProperty(this.model.woData, 'workOrderType') === 'single'
        ) {
          let { singleResource } = this.model.woData || {}
          if (!isEmpty(singleResource)) {
            if (singleResource.resourceTypeEnum === 'ASSET') {
              this.$setProperty(
                this.model.woData,
                'resourceType',
                'ASSET_CATEGORY'
              )
              this.$setProperty(this.model.woData, 'selectedResourceList', [
                workorder.resource,
              ])
              let { category } = workorder.resource || {}
              this.$setProperty(
                this.model.woData,
                'assetCategoryId',
                category.id || -1
              ...
              singleResource.id === singleResource.siteId &&
              singleResource.spaceId === singleResource.siteId &&
              singleResource.floorId === -1 &&
              singleResource.buildingId === -1
            ) {
              this.$setProperty(this.model.woData, 'resourceType', 'ALL_SITES')
              this.$setProperty(this.model.woData, 'selectedResourceList', [
                workorder.resource,
              ])
            } else if (
              singleResource.buildingId > -1 &&
              singleResource.floorId === -1 &&
              singleResource.id === singleResource.buildingId &&
              singleResource.buildingId === singleResource.spaceId
            ) {
              this.$setProperty(this.model.woData, 'resourceType', 'BUILDINGS')
              this.$setProperty(this.model.woData, 'selectedBuildingList', [
                workorder.resource,
              ])
            } else if (
              singleResource.floorId > 0 &&
              singleResource.id === singleResource.floorId &&
              singleResource.floorId === singleResource.spaceId
            ) {
              this.$setProperty(this.model.woData, 'resourceType', 'ALL_FLOORS')
              this.$setProperty(this.model.woData, 'selectedFloorList', [
                workorder.resource,
              ])
              // TODO: set building ID if it's part of building ID
              if (singleResource.buildingId > 0) {
                this.$set(
                  this.model.woData,
                  'selectedBuilding',
                  singleResource.buildingId
                )
              }
            } else if (
              (singleResource.floorId > 0 ||
                singleResource.buildingId > 0 ||
                singleResource.siteId > 0) &&
              singleResource.spaceId != singleResource.siteId &&
              singleResource.spaceId != singleResource.buildingId &&
              singleResource.spaceId != singleResource.floorId
            ) {
              this.$setProperty(
                this.model.woData,
                'resourceType',
              ...
              this.$setProperty(this.model.woData, 'selectedSpaceList', [
                workorder.resource,
              ])
              let space = await this.fetchSpace(workorder.resource.id)
              if (!isEmpty(space)) {
                let { spaceCategory } = space || {}

                if (isEmpty(spaceCategory) || isEmpty(spaceCategory.id)) {
                  this.$message({
                    message: this.$t(
                      'maintenance.old_pm.please_update_category_of_the_selected_space'
                    ),
                    type: 'error',
                  })
                  this.isSaving = false
        ...
        this.$setProperty(this.model.woData, 'resourceList', [
          workorder.resource,
        ])
        //this.$setProperty(this.model.woData, 'selectedResourceList', [workorder.resource])
      ...
      if (workorder.site && workorder.site.id) {
        //workorder.siteId = workorder.site.id
        workorder.sites = [workorder.site.id] // TODO: set site ID as array
        delete workorder.site
      }
      ...
      // let isSingleWoAssetType = // TODO: commented out this as it isn't required
      //   this.model.woData.workOrderType === 'single' &&
      //   this.model.woData.singleResource.resourceTypeEnum === 'ASSET'
      ...
      preventiveFormData.pmCreationType = 3 // TODO: updated creation type
      preventiveFormData.siteIds = workorder.sites // TODO: updated sites
      preventiveFormData.baseSpaceId =
        this.model.woData.selectedBuilding ||
        this.getSelectedSiteId(workorder.sites) ||
        -1 // TODO: update the baseSpaceId
      preventiveFormData.pmIncludeExcludeResourceContexts = []
      preventiveFormData.enableSkipTriggers = true
      preventiveFormData.defaultAllTriggers = true
      ...
      if (this.model.woData.resourceType === 'ALL_FLOORS') {
        ...
        this.model.woData.resourceList.forEach(x => {
          // TODO: change selectedSpaceList to resourceList
          preventiveFormData.pmIncludeExcludeResourceContexts.push({
            resourceId: x.id,
            isInclude: true,
            parentType: 1,
        ...
        this.model.woData.selectedBuildingList.forEach(x => {
          preventiveFormData.pmIncludeExcludeResourceContexts.push({
            resourceId: x.id, // TODO: push the ID
            isInclude: true,
            parentType: 1,
          })
        })
    ...
    async fetchSpace(spaceId) {
      let { space, error } = await API.fetchRecord('space', { id: spaceId })
      if (!error) {
        return space
      } else {
        this.$message.error('Error occurred while fetching space.')
      }
    },
    getSelectedSiteId(sites) {
      ...
      return isEmpty(sites) ? null : sites.length == 1 ? sites[0] : null
    },
  },
  watch: {
  ...
  computed: {
    ...
    siteName() {
      if (
        ...
        let site = this.siteOptions.find(
          i => this.model.woData.woModel.site.id === i.value
        )
        if (site) {
          return site.label
        }
