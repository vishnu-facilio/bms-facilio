...
import WoForm from 'pages/workorder/preventive/PmWoForm'
import TaskForm from 'pages/workorder/preventive/pmformtask'
import PreRequestForm from 'pages/workorder/preventive/pmformprerequest'
import TriggerForm from 'pages/workorder/preventive/pmformtrigger'
import OldTriggerForm from 'pages/workorder/preventive/pmformtriggerold'
import ReviewForm from 'pages/workorder/preventive/PmReview'
import OldReviewForm from 'pages/workorder/preventive/OldPmReview'
import ResourcePlan from 'pages/workorder/preventive/PmResourcePlan'
  ...
  methods: {
    ...
    edited() {
      ...
      if (isWebTabsEnabled()) {
        ...
        this.$router.replace({
          path: `/app/wo/planned/summary/${edit}`,
        })
      }
    },
    fetchPmDetail() {
      ...
      if (this.model.isEdit) {
        ...
        this.$http.get(`v2/workorders/pmDetail?id=${id}`).then(response => {
          ...
          if (woData.siteId > -1) {
            ...
            woData.site.id = response.data.preventivemaintenance.siteId
          }
          delete woData.siteId

          woData.duration = this.$util.unixTimeToDaysHoursPoint5(
            woData.duration
          )
          woData.estimatedWorkDuration = this.$util.unixTimeToDaysHoursPoint5(
          ...
          this.model.woData.workOrderType =
            response.data.preventivemaintenance.pmCreationType === 1
              ? 'single'
              : 'bulk'
          ...
          this.$nextTick(() => {
            ...
            if (response.data.preventivemaintenance.pmCreationType === 2) {
