  ...
  >
    <div class="position-relative">
      ...
      <el-row>
        ...
        <template v-if="data">
          <el-col :span="24" class="space-title" v-if="data.value.value">{{
            `Total no of workorders ${data.value.value}`
          }}</el-col>
          <el-col
            :span="24"
  ...
  computed: {
    tooltipData() {
      if (this.area && this.area.length) {
        if (this.area[0] && this.area[0].tooltipData) {
          return this.area[0].tooltipData
        }
      }
      return null
    },
