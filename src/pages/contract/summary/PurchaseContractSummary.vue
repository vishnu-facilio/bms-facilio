                  ...
                  >
                    <div v-if="!showRevisionHistory" class="pT10">
                      <div
                        ...
                        @click="getSummaryLink(purchaseContract.id)"
                        v-for="purchaseContract in purchaseContracts"
                        :key="purchaseContract.id"
                        v-bind:class="{
                          fcactivelist: id === purchaseContract.id,
                        }"
                      >
                        <div
                        ...
                        <img
                          ...
                          width="15"
                          height="15"
                          v-bind:class="{ show: id === purchaseContract.id }"
                        />
                      </div>
                    </div>
                    <div v-else-if="showRevisionHistory" class="pT10">
                      <div
                        class="label-txt-black pb20 flex-middle justify-content-space"
                        @click="getHistorySummaryLink(purchaseContract.id)"
                        v-for="purchaseContract in revisionHistoryJSON.list"
                        :key="purchaseContract.id"
                        v-bind:class="{
                          fcactivelist: id === purchaseContract.id,
                        }"
                        ...
                        >
                          ...
                          >
                            ...
                            <el-table-column label="COST">
                              <template v-slot="scope">
                                <currency :value="scope.row.cost"></currency>
                              </template>
                            </el-table-column>
                            <el-table-column
                              v-if="isContractEditable(summaryData.status)"
            ...
            >
              ...
              <td class="module-builder-td">
                <el-input
                  ...
                  type="number"
                  :min="0"
                  v-model="lineItemsFormObject.lineItems[lineindex].quantity"
...
export default {
  ...
  computed: {
    ...
    purchaseContracts() {
      return this.$store.state.purchasecontract.purchaseContracts
    },
    getCurrentModule() {
      return 'purchasecontracts'
    },
    canLoadMore() {
      return this.$store.state.purchasecontract.canLoadMore
    },
    scrollDisabled() {
    ...
    currentViewDetail() {
      if (this.$route.query.search) {
        return {
          displayName: 'Filtered Purchase Contract',
          name: 'filteredpurchasecontract',
        }
      }
    ...
    searchQuery() {
      return this.$store.state.purchasecontract.quickSearchQuery
    },
  },
  watch: {
    purchaseContracts(newVal) {
    ...
    '$route.query.search': {
      handler(value) {
        let viewIndex = this.views.findIndex(
          view => view.name === 'filteredpurchasecontract'
        )
  ...
  methods: {
    loadForm() {
      let self = this
      let url = '/getFormMeta?formNames=purchaseContractForm'
      this.$http
        .get(url)
        .then(response => {
    ...
    updateStatus(status, message) {
      ...
      this.$http
        .post('v2/purchasecontract/addOrUpdate', param)
      ...
      if (col.label === 'ITEM NAME') {
        if (isWebTabsEnabled()) {
          ...
          if (name) {
            this.$router.push({
              name,
              params: { id: row.itemType.id, viewname: 'all' },
            })
          }
        } else {
          this.$router.push({
    ...
    loadSummary() {
      let self = this
      let url = '/v2/purchasecontract/getById'
      let params = {
        recordId: this.id,
      }
      if (this.id) {
        this.$http
          ...
          .then(response => {
            self.summaryData =
              response.data.result && response.data.result.purchasecontract
                ? response.data.result.purchasecontract
                : []
            let title =
    ...
    quickSearch() {
      this.$store.dispatch(
        'purchasecontract/updateSearchQuery',
    ...
    loadData(loadMore) {
      ...
      self.$store
        .dispatch('purchasecontract/fetchPurchaseContracts', queryObj)
        .then(function() {
          self.loading = false
    ...
    getSummaryLink(id) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = '/app/ct/purchasecontracts/' + viewname + '/summary/' + id
    ...
    switchCategory(index) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = '/app/ct/purchasecontracts/' + index.name + '/summary/' + null
    ...
    handleCommand(command) {
      if (['edit', 'duplicate'].includes(command)) {
        ...
        if (isWebTabsEnabled()) {
          ...
          this.$router.push({
            path: '/app/ct/purchasecontracts/edit/' + id,
            query,
          })
        }
      } else if (command === 'delete') {
        this.delete()
      } else if (command === 'createpo') {
        this.convertContractToPo()
      } else if (command === 'revise') {
        this.reviseContract(this.summaryData, 'purchasecontract')
    ...
    delete() {
      let url = '/v2/purchasecontract/delete'
      let params = {
        ...
        .then(response => {
          if (response.data.responseCode === 0) {
            ...
            if (isWebTabsEnabled()) {
              ...
              this.$router.push({
                path: '/app/ct/purchasecontracts/all',
              })
            }
          }
        })
    ...
    addLineItemtoContract() {
      for (let i = this.lineItemsFormObject.lineItems.length - 1; i >= 0; i--) {
        ...
        if (
          this.lineItemsFormObject.lineItems[i].quantity === 0 ||
          this.lineItemsFormObject.lineItems[i].quantity === '' ||
          this.lineItemsFormObject.lineItems[i].quantity === null ||
    ...
    convertContractToPo() {
      let param = {
        vendor: { id: this.summaryData.vendor.id },
        lineItems: [],
        billToAddress: {
          street: null,
          city: null,
      ...
      for (let i in this.summaryData.lineItems) {
        if (this.summaryData.lineItems[i].inventoryType === 1) {
          param.lineItems.push({
            quantity: this.summaryData.lineItems[i].quantity,
            unitPrice: this.summaryData.lineItems[i].unitPrice,
            inventoryType: 1,
            itemType: { id: this.summaryData.lineItems[i].itemType.id },
          })
        } else if (this.summaryData.lineItems.inventoryType === 2) {
          param.lineItems.push({
            quantity: this.summaryData.lineItems[i].quantity,
            unitPrice: this.summaryData.lineItems[i].unitPrice,
            inventoryType: 2,
            toolType: { id: this.summaryData.lineItems[i].toolType.id },
          })
        }
      }
      this.$http
        ...
        .then(response => {
          ...
          param.billToAddress.city = address.city
          param.billToAddress.state = address.state
          param.billToAddress.street = address.street
          param.billToAddress.country = address.country
          param.billToAddress.zip = address.zip

          if (isWebTabsEnabled()) {
            let { name } =
              findRouteForModule('purchaseorder', pageTypes.CREATE) || {}
            name &&
              this.$router.push({
                name,
                query: { contractContext: JSON.stringify(param) },
              })
          } else {
            this.$router.push({
              path:
