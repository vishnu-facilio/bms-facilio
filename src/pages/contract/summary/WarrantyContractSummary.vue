                  ...
                  >
                    <div v-if="!showRevisionHistory" class="pT10">
                      <div
                        ...
                        @click="getSummaryLink(wc.id)"
                        v-for="wc in warrantyContracts"
                        :key="wc.id"
                        v-bind:class="{ fcactivelist: id === wc.id }"
                      >
                        <div
                          class="width220px textoverflow-ellipsis"
                          :title="wc.name ? wc.name : '[#' + wc.id + ']'"
                          v-tippy="{
                            placement: 'top',
                      ...
                      >
                        ...
                        >
                          <el-table
                            ...
                            "
                            :default-sort="{
                              prop: 'summaryData.lineItems.service.name',
                              order: 'descending',
                            }"
                          ...
                          >
                            <el-table-column
                              prop="service.name"
                              :label="$t('common.products._service')"
                            ></el-table-column>
                            <el-table-column
                              :label="$t('common.header.item_coverage')"
                            >
                              <template v-slot="scope">
                                ...
                                >
                                  <el-input
                                    ...
                                    "
                                    class="fc-input-full-border2 width50px inventory-input-width text-right pL20 pr-summary"
                                  ></el-input>
                                </div>
                                <div v-else>
                                  <div>
                            ...
                            <el-table-column
                              :label="$t('common.header.tool_coverage')"
                            >
                              <template v-slot="scope">
                                <div
                                  v-if="isContractEditable(summaryData.status)"
                            ...
                            <el-table-column
                              :label="$t('common.header.labor_coverage')"
                            >
                              <template v-slot="scope">
                                <div
                                  v-if="isContractEditable(summaryData.status)"
                      ...
                      >
                        ...
                        >
                          <contract-associated-assets
                            ...
                            :associateAssetsVisibility.sync="associateAssetsVisibility"
                            :purchaseAssetVisibility.sync="purchaseAssetVisibility"
                        ...
                        >
                          ...
                          >
                            <comments
                              ...
                              :placeholder="$t('common._common.add_note_btn')"
                              :btnLabel="$t('common._common.add_note_btn')"
                              :record="summaryData"
                              :notify="false"
                            ></comments>
                          </div>
          ...
          <table class="setting-list-view-table store-table">
            ...
            <tbody :key="summaryData.id">
              ...
              >
                ...
                <td class="module-builder-td">
                  <el-input
                    :placeholder="$t('common.header.tool_coverage')"
                    type="number"
                    :max="100"
                    :min="0"
                    v-model="
                ...
                <td class="module-builder-td">
                  <el-input
                    :placeholder="$t('common.header.labor_coverage')"
                    type="number"
                    :max="100"
                    :min="0"
                    v-model="
                ...
                <td>
                  <div class="visibility-hide-actions export-dropdown-menu">
                    <span
                      @click="
                        addWarrantyLineEntry(lineItemsFormObject.lineItems)
                      "
...
export default {
  ...
  computed: {
    ...
    canLoadMore() {
      return this.$store.state.warrantycontract.canLoadMore
    },
    scrollDisabled() {
    ...
    currentView() {
      if (this.$route.query.search) {
        this.views.push({
          displayName: 'Filtered Warranty Contracts',
          name: 'filteredwarrantycontracts',
        })
        return 'Filtered Warranty Contracts'
      }
    ...
    searchQuery() {
      return this.$store.state.warrantycontract.quickSearchQuery
    },
  },
  watch: {
    warrantyContracts(newVal) {
  ...
  methods: {
    loadForm() {
      let self = this
      let url = '/getFormMeta?formNames=warrantyContractForm'
      this.$http
        .get(url)
        .then(response => {
    ...
    updateStatus(status, message) {
      ...
      this.$http
        .post('v2/warrantycontract/addOrUpdate', param)
          ...
          .then(function(response) {
            if (response.data.responseCode === 0) {
              self.$message.success(
                this.$t('common.products.description_updated_successfully')
              )
    ...
    loadSummary() {
      let self = this
      let url = '/v2/warrantycontract/getById'
      let params = {
        recordId: this.id,
      }
      if (this.id) {
        this.$http
          ...
          .then(response => {
            self.summaryData =
              response.data.result && response.data.result.warrantycontract
                ? response.data.result.warrantycontract
                : []
            let title =
    ...
    quickSearch() {
      this.$store.dispatch(
        'warrantycontract/updateSearchQuery',
    ...
    loadData(loadMore) {
      ...
      self.$store
        .dispatch('warrantycontract/fetchWarrantyContracts', queryObj)
        .then(function() {
          self.loading = false
    ...
    getSummaryLink(id) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = '/app/ct/warrantycontracts/' + viewname + '/summary/' + id
    ...
    switchCategory(index) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = '/app/ct/warrantycontracts/' + index.name + '/summary/' + null
    ...
    handleCommand(command) {
      if (['edit', 'duplicate'].includes(command)) {
        ...
        if (isWebTabsEnabled()) {
          ...
          this.$router.push({
            path: '/app/ct/warrantycontracts/edit/' + id,
            query,
          })
        }
        ...
        this.reviseContract(this.summaryData, 'warrantycontract')
    ...
    delete() {
      let self = this
      let url = '/v2/warrantycontract/delete'
      let params = {
        ...
        .then(response => {
          if (response.data.responseCode === 0) {
            ...
            if (isWebTabsEnabled()) {
              ...
              self.$router.push({
                path: '/app/ct/warrantycontracts/all',
              })
            }
          }
        })
    ...
    removeWarrantyLineEntry(list, index) {
      list.splice(index, 1)
    },
    isValid(val) {
    ...
    addWarrantyLine() {
      for (let i = this.lineItemsFormObject.lineItems.length - 1; i >= 0; i--) {
        if (!this.isValid(this.lineItemsFormObject.lineItems[i].service.id)) {
      ...
      this.$http
        .post('v2/warrantycontract/addOrUpdateLineItem', param)
        .then(response => {
          if (response.data.responseCode === 0) {
            this.addLineItemsVisibility = false
            this.loadSummary()
