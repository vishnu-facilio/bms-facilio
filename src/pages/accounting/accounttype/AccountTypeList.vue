  ...
  >
    <template #header>
      ...
      <template v-if="!showSearch">
        <button
          ...
          @click="openNewForm()"
        >
          {{ $t('tenant.accounting.new_acc_type') }}
        </button>
      </template>
    </template>
    ...
    <template #sub-header-actions>
      <template v-if="!isEmpty(moduleRecordList)">
        ...
        <el-tooltip
          v-if="$hasPermission(`budget:EXPORT`)"
          effect="dark"
    ...
    <template #content>
        ...
        >
          ...
          >
            ...
            <div class="q-item-label nowo-label">
              {{ $t('tenant.accounting.no_data_acc_type') }}
            </div>
          </div>
          <template v-else>
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/ac/accounttype/viewmanager`"
...
import ModuleForm from './AccounttypeForm'
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Account Type'
    },
    appliedFilters() {
      let {
        $route: { query },
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(
          message || 'Error Occured while fetching Account Type list'
        )
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`tenant.accounting.delete_acc_type`),
        message: this.$t(`tenant.accounting.delete_confirmation_acc_type`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          let { message } = error
          this.$message.error(
            message || 'Error Occured while deleting Account Type'
          )
          this.loading = false
        } else {
          this.$message.success(
            this.$t(`tenant.accounting.delete_success_acc_type`)
          )
