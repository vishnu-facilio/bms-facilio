<template>
  <CommonListLayout
    ...
    pathPrefix="/app/ac/chartofaccount/"
  >
    <template #header>
    ...
    <template #content>
        ...
        >
          ...
          >
            ...
            <div class="q-item-label nowo-label">
              {{ $t('tenant.accounting.no_data_coa') }}
            </div>
          </div>
          <template v-else>
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/ac/chartofaccount/viewmanager`"
...
import ModuleForm from './ChartOfAccountForm'
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Chart of Account'
    },
    appliedFilters() {
      let {
        $route: { query },
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(
          message || 'Error Occured while fetching Chart of Account list'
        )
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`tenant.accounting.delete_coa`),
        message: this.$t(`tenant.accounting.delete_confirmation_coa`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          let { message } = error
          this.$message.error(
            message || 'Error Occured while deleting Chart of Account'
          )
          this.loading = false
        } else {
          this.$message.success(this.$t(`tenant.accounting.delete_success_coa`))
