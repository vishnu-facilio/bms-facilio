    ...
    <template #header>
      ...
      <template v-if="!showSearch">
        <button
          ...
          @click="addRecord()"
        >
          {{ $t('tenant.accounting.new_budget') }}
        </button>
      </template>
    </template>

    <template #sub-header-actions>
      <template v-if="!$validation.isEmpty(moduleRecordList)">
        <pagination
          :total="listCount"
          :perPage="50"
          :skipTotalCount="true"
        ...
        >
          ...
          >
            ...
            <div class="q-item-label nowo-label">
              {{ $t('tenant.accounting.no_data_budget') }}
            </div>
          </div>
          <template v-else>
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Budget'
    },
    appliedFilters() {
      let {
        $route: { query },
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(
          message || 'Error Occured while fetching budget list'
        )
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`tenant.accounting.delete`),
        message: this.$t(`tenant.accounting.delete_confirmation`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          let { message } = error
          this.$message.error(message || 'Error Occured while deleting budget')
          this.loading = false
        } else {
          this.$message.success(this.$t(`tenant.accounting.delete_success`))
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-budget' })
      }
    },
    editRecord({ id = null } = {}) {
      if (isWebTabsEnabled()) {
