      ...
      <div v-else class="flex flex-row">
        ...
        >
          <SummarySideBar
            ...
            class="group-summary-list"
          >
            <template #title>
              <div
                v-show="!isTenantGroup"
        ...
        >
          ...
          >
            ...
            <div class="q-item-label nowo-label">
              {{ $t('controls.controlGroups.no_data') }}
            </div>
          </div>
          <div v-else>
            <el-table
...
export default {
  props: ['viewname', 'id', 'moduleName', 'tenantId'],
  components: {
    CommonListLayout,
    SummarySideBar,
    Pagination,
  ...
  methods: {
    async loadRecords() {
      ...
      if (!isEmpty(this.appliedFilters)) {
        ...
        if (this.isTenantGroup && !isEmpty(this.tenantId)) {
          filters = {
            ...filters,
      ...
      if (error) {
        let {
          message = this.$t(
            'common._common.error_occured_while_fetching_control_groups_list'
          ),
        } = error
    ...
    back() {
      ...
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ path: '/app/co/group/all' })
      }
    },
    editRecord(id) {
      if (isWebTabsEnabled()) {
    ...
    async deleteRecord(id) {
      let value = await this.$dialog.confirm({
        title: this.$t(`controls.controlGroups.delete_group`),
        message: this.$t(`controls.controlGroups.delete_group_confirmation`),
        rbDanger: true,
