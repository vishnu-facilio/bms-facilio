            ...
            >
              ...
              <div v-show="folder.expand" class="rfolder-children">
                ...
                <div
                  ...
                  :key="gId"
                  @click="redirectToGraphics(graphic.id)"
                >
                  {{ graphic.name }}
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    openGraphics() {
      if (!this.$route.params.graphicsid) {
        for (let folder of this.graphicsFolderList) {
          if (folder.graphics.length > 0) {
            this.redirectToGraphics(folder?.graphics[0]?.id)
            folder.expand = true
            return
          }
        }
      ...
      this.$http
        ...
        .then(response => {
          if (response.data.responseCode === 0) {
            ...
            if (!this.$route.params.graphicsid) {
              this.redirectToGraphics(this.graphicsList[0]?.id)
            }
            this.loading = false
          } else {
            this.$message.error(response.data.message)
    ...
    redirectToGraphics(graphicId) {
      let actualPath = `/app/co/graphics/view/${graphicId}`
      if (isWebTabsEnabled()) {
        let { path } = findRouteForTab(tabTypes.CUSTOM, {
          config: { type: 'graphics' },
        })
        actualPath = `/${getApp()?.linkName}/${path}/view/${graphicId}`
      }
