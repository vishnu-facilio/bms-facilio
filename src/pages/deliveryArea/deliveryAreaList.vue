<template>
  <CommonListLayout
    ...
    pathPrefix="/app/dl/deliveryArea/"
  >
    <template #header>
      ...
      <template v-if="!showSearch">
        <button
          ...
          @click="addRecord()"
        >
          {{ $t('deliveries.deliveryArea.new_deliveryArea') }}
        </button>
      </template>
    </template>
    ...
    <template #content>
        ...
        >
          ...
          >
            ...
            <div class="q-item-label nowo-label">
              {{ $t('deliveries.deliveryArea.no_data_deliveryArea') }}
            </div>
          </div>
          <template v-else>
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/dl/deliveryArea/viewmanager`"
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Delivery Areas'
    },
    appliedFilters() {
      let {
        $route: { query },
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(
          message || 'Error Occured while fetching delivery areas list'
        )
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`deliveries.deliveryArea.delete_deliveryArea`),
        message: this.$t(
          `deliveries.deliveryArea.delete_confirmation_deliveryArea`
        ),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          let { message } = error
          this.$message.error(
            message || 'Error Occured while deleting delivery area'
          )
          this.loading = false
        } else {
          this.$message.success(
            this.$t(`deliveries.deliveryArea.delete_success_deliveryArea`)
          )
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-deliveryArea' })
      }
    },
    editRecord({ id = null } = {}) {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'edit-deliveryArea', params: { id } })
      }
    },
