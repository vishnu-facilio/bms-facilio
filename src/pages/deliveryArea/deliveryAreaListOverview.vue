            ...
            <ul class="sp-ul mT0">
              ...
              >
                ...
                >
                  <el-row>
                    <el-col :span="24">
                      <div class="facility-id">{{ `#${row.id}` }}</div>
                      <div
                        class="summary-bold-text1 truncate-text mT5"
  ...
  methods: {
    ...
    switchView(view) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = `/app/dl/deliveryArea/${view.name}/${null}/overview`
    ...
    async loadData(loadMore, forceFetch) {
      ...
      if (error) {
        let {
          message = 'Error Occured while fetching delivery area list',
        } = error
        ...
        if (forceFetch) {
          ...
          if (loadMore) {
            this.deliveryAreaList = [...(this.deliveryAreaList || []), ...list]
          }
        }
        let listCount = this.$getProperty(meta, 'pagination.totalCount', null)
        this.canLoadMoreList = listCount > (this.deliveryAreaList || []).length
      }
      this.loading = false
