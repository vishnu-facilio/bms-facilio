  ...
  >
    <template #header>
      ...
      <CustomButton
        ...
        @onError="() => {}"
      ></CustomButton>
      <template v-if="$hasPermission('tenantcontact:CREATE')">
    ...
    <template #sub-header-actions>
      <template v-if="!isEmpty(records)">
        ...
        <template v-if="$hasPermission('tenantcontact:EXPORT')">
        ...
        <div class="cm-list-container">
          ...
          >
            <div class="action-btn-slide btn-block">
              <button
                v-if="$hasPermission('tenantcontact:DELETE')"
