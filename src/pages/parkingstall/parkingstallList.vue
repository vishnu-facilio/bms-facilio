<template>
  <CommonListLayout
    ...
    pathPrefix="/app/ps/parkingstall/"
  ...
  >
    <template #header>
      ...
      <template v-if="!showSearch">
        <button
          ...
          @click="addRecord()"
        >
          {{ $t('space.parkingstall.new_parkingstall') }}
        </button>
      </template>
    </template>
    ...
    <template #content>
        ...
        >
          ...
          >
            ...
            <div class="q-item-label nowo-label">
              {{ $t('space.parkingstall.no_data_parkingstall') }}
            </div>
          </div>
          <template v-else>
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/ps/parkingstall/viewmanager`"
  ...
  methods: {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`space.parkingstall.delete_parkingstall`),
        message: this.$t(`space.parkingstall.delete_confirmation_parkingstall`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          ...
          this.$message.success(
            this.$t(`space.parkingstall.delete_success_parkingstall`)
          )
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-parkingstall' })
      }
    },
    editRecord({ id = null } = {}) {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'edit-parkingstall', params: { id } })
      }
    },
