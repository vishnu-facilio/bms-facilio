  ...
  methods: {
    ...
    switchView(view) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = `/app/ps/parkingstall/${view.name}/${null}/overview`
    ...
    async loadData(loadMore, forceFetch) {
      ...
      if (error) {
        ...
        if (forceFetch) {
          this.parkingstallList = list
        } else {
          if (loadMore) {
            this.parkingstallList = [...(this.parkingstallList || []), ...list]
          }
        }
        let listCount = this.$getProperty(meta, 'pagination.totalCount', null)
        this.canLoadMoreList = listCount > (this.parkingstallList || []).length
      }
      this.loading = false
