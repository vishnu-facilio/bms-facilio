      ...
      >
        <div class="graphics-applyto-dialog">
          ...
          >
            <el-radio :label="1" class="fc-radio-btn"
              >All Category Assets</el-radio
            >
            <el-radio :label="2" class="fc-radio-btn">Specific Assets</el-radio>
            <el-radio :label="3" class="fc-radio-btn"
              >Filter Based Assets</el-radio
            >
          </el-radio-group>
          <el-row v-if="applyToData.applyToType === 2">
            <el-col :span="12" class="mT30">
              <p class="fc-input-label-txt">Applicable Assets</p>
              <div
          ...
          <el-row v-if="applyToData.applyToType === 3">
            <el-col>
              <p class="fc-input-label-txt pT20 pB0">Filters</p>
          ...
          <el-row>
            <el-col :span="12" class="mT30">
              <p class="fc-input-label-txt">Default Asset</p>
              <div
                class="el-select-block form-input fc-input-full-border-select2"
              >
                <el-select
        ...
        <div class="modal-dialog-footer">
          ...
          <el-button
            ...
            >{{ saveLoading.applyTo ? 'SAVING...' : 'SAVE' }}</el-button
          >
        </div>
      </el-dialog>
    </div>
...
export default {
  ...
  methods: {
        ...
        util
          ...
          .then(response => {
            ...
            if (response.assets && response.assets.length === 0) {
              if (errorMessage) {
                this.$message.error('No Assets Found for the Applied Criteria')
              }
            }
          })
          .catch(error => {
    ...
    deleteGraphics(index, graphics) {
      let confirmed = confirm('Are you sure want to delete this Graphics?')
      if (confirmed) {
        this.$http
          .post('/v2/graphics/delete', { recordId: graphics.id })
          .then(response => {
            if (response) {
              this.$router.push({ path: '/app/em/graphics/' })
              this.$emit('deleted')
            }
          })
      }
    ...
    saveApplyTo() {
      ...
      if (this.applyToData.applyToType > 0) {
        if (
          ...
          this.$message.error('Kindly Enter Default Asset')
          return
        }
        if (this.applyToData.applyToType === 2) {
          if (this.applyToData.applyToAssetIds.length === 0) {
            this.$message.error('Select Atleast One Asset')
            return
          }
        }
        if (this.applyToData.applyToType === 3) {
          if (this.staticCriteria !== this.applyToData.criteria) {
            this.$message.error('Kindly Load Asset For Criteria')
            return
          }
        }
      } else {
        this.$message.error('Kindly Input a value')
        return
      }
