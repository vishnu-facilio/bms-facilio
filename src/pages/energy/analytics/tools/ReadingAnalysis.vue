  ...
  <div>
      ...
      <div class="row">
        <div class="col-7">
          ...
          <div class="analytics-page-header-filters">
            ...
            <f-data-point-adder
              :showAddPoint="analyticsConfig.dataPoints.length"
              ref="readingAdder"
              type="reading"
    ...
    <div ref="chartSection" class="self-center fchart-section scrollable">
      <div v-if="!analyticsConfig.dataPoints.length" class="text-center">
        <div class="p15">Please select a reading to analyze.</div>
        <el-button
          size="small"
          class="freport-btn"
          @click="$refs.readingAdder.selectDataPoint()"
          >Select Reading</el-button
        >
      </div>
      <f-analytic-report
...
export default {
  ...
  mounted() {
    let config = this.loadAnalyticsConfig()
    if (this.alarmId) {
      this.loadAlarmMeta()
    } else {
  ...
  methods: {
      ...
      this.$http
        ...
        .then(response => {
          if (response.data && response.data.reportMeta) {
            if (response.data.reportMeta.baselineId > 0) {
              this.selectedBaseLineId = response.data.reportMeta.baselineId
              this.selectBaseLine()
            }
            this.initData(response.data.reportMeta)
          }
        })
    },
    initData(config) {
      ...
      this.analyticsConfig = this.$helpers.extend(this.analyticsConfig, config)
      if (this.$refs.readingAdder) {
        this.$refs.readingAdder.setDataPoints(
          this.analyticsConfig.dataPoints.filter(d => d.type !== 'derivation')
        )
      }
    },
    dataPointsChanged(dataPoints) {
      ...
      this.setDerivationPoints()
      console.log('###### ', this.analyticsConfig)
    },
    setDerivationPoints() {
      let datapoints = this.$helpers.cloneObject(this.derivations)
      this.analyticsConfig.dataPoints = this.analyticsConfig.dataPoints
        .filter(d => d.type !== 'derivation')
        .concat(datapoints)
    },
    printReport() {
      setTimeout(function() {
