  ...
  <div>
        ...
        <div class="col-7">
          ...
          <div class="analytics-page-header-filters">
            <el-select
              ...
              filterable
              v-model="enpiDataPoint.buildingId"
            ...
            <el-select
              @change="selectEnPI"
              v-if="enpiDataPoint.buildingId"
              filterable
              v-model="enpiDataPoint.readingFieldId"
              placeholder="Select EnPI"
            >
              <el-option-group label="EnPI">
                <el-option
                  v-for="(enpi, index) in buildingEnPIList"
                  :key="index"
                  :label="enpi.name"
                  :value="enpi.readingFieldId"
                >
                  <span style="float: left">{{ enpi.name }}</span>
                  <span
                    style="float: right; color: #8492a6; font-size: 13px; margin-left: 15px;"
                    >{{
                      $helpers.capitalize(enpi.frequencyEnum.toLowerCase())
                    }}</span
                  >
                </el-option>
              </el-option-group>
    ...
    <div ref="chartSection" class="self-center fchart-section scrollable">
      <div v-if="!analyticsConfig.dataPoints.length" class="text-center">
        <div class="p15">Please select an EnPI to analyze.</div>
      </div>
      <f-analytic-report
...
export default {
  ...
  created() {
    this.$store.dispatch('loadBuildings').then(() => {
      if (this.$store.getters.getBuildingsPickList().length) {
        this.enpiDataPoint.buildingId = this.$store.getters.getBuildingsPickList()[0]
      }
      this.loadEnPI()
    })
  },
  ...
  computed: {
    ...
    buildingEnPIList() {
      if (this.enpiList && this.enpiDataPoint.buildingId) {
        return this.enpiList.filter(
          en => en.resourceId === parseInt(this.enpiDataPoint.buildingId)
        )
      }
      return []
    },
  ...
  methods: {
    loadEnPI() {
      let self = this
      self.$http.get('/reading/allformulas?type=1').then(function(response) {
    ...
    selectBuilding() {
      this.enpiDataPoint.parentId = this.enpiDataPoint.buildingId
      this.enpiDataPoint.readingFieldId = ''
    },
    selectEnPI() {
      let enpi = this.enpiList.find(
        en => en.readingFieldId === this.enpiDataPoint.readingFieldId
      )
      this.enpiDataPoint.name = enpi.name
      this.enpiDataPoint.readingField = enpi.readingField

      let period = 0
      let chartType = 'bar'
      if (enpi.frequencyEnum === 'DAILY') {
        period = 12
      } else if (enpi.frequencyEnum === 'WEEKLY') {
        period = 11
      } else if (enpi.frequencyEnum === 'MONTHLY') {
        period = 10
      } else {
        period = 0
        chartType = 'area'
      ...
      this.analyticsConfig.chartType = chartType
      this.analyticsConfig.dataPoints = [
        this.$helpers.cloneObject(this.enpiDataPoint),
      ]
    },
  },
}
