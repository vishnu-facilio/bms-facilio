<template>
  <div class="row">
    <div class="col-12 analytic-summary">
      ...
      ></iframe>
      <div
        class="analytics-page-header heatmap-analytics new-heatmap-analytics"
      >
        <div class="row">
          <div class="col-7">
            <div class="analytics-page-header-filters">
              ...
              <el-select
                ...
                filterable
                :placeholder="$t('common.wo_report.select_meter')"
                class="width200px"
              >
          ...
          <div class="col-5" style="text-align: right;">
            <el-button
              class="freport-btn analytics-save-as-repor new-analytics-Rbtn"
              :disabled="isActionsDisabled"
              @click="visibility.saveAsDialog = true"
              >{{ $t('common.wo_report.save_as_report') }}</el-button
            >
            <div class="new-analytics-Rbtn analytics-page-options">
              <el-dropdown
              ...
              <el-button
                ...
                type="text"
                @click="printReport"
                :title="$t('common.wo_report.print')"
                data-position="bottom"
                data-arrow="true"
                v-tippy
                class="fc-chart-btn"
      ...
      <div ref="chartSection" class="self-center fchart-section scrollable">
        <div v-if="!analyticsConfig.dataPoints.length" class="text-center">
          <div class="p15">
            {{ $t('common.wo_report.please_select_meter') }}
          </div>
        </div>
        <f-analytic-report
          :config.sync="analyticsConfig"
          v-else
          class="detail-report-align"
        ></f-analytic-report>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    loadAssets(id) {
      this.loading = true
      let self = this
      this.$util.loadAsset({ withReadings: true }).then(response => {
        if (response.assets) {
          self.assetList = response.assets
