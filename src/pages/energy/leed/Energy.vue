<template>
  <div class="scrollable">
    <div class="row p25 le-background-color-2">
      ...
      >
        <span class="">Electric Utility Provider</span>
      </div>
      <div v-if="Utility">
      ...
      >
        <div class="row">
          ...
          >
            <span v-if="loadedmetername == meter.name" class="le-color-pink">{{
              meter.name
            }}</span
            ><span v-else>{{ meter.name }}</span>
          </button>
          <!--button class="le-button le-button3 uppercase">dewa</button -->
          <!--     <button @click="$refs.DataManualModel.open()" class="le-button le-button3 border-right-radious" v-tippy
                data-html="#utility-more-options1"
                data-distance="15"
                data-theme="light"
                ...
                data-trigger="click"
                data-arrow="true"><q-icon name="add" class=" leed-color-1 " size="18px"></q-icon></button> -->
          <button
            @click="$refs.DataManualModel.open()"
    ...
    <div class="row p25">
      <div
        class="col-12 col-sm-12 col-lg-12 col-md-12 text-left le-content-label"
      >
        <span class="">Onsite Power</span>
      </div>
      <div class="col-6 col-sm-6 col-lg-6 col-md-6 text-left">
        <span class="le-content-sublabel"
          >Choose from the below onsite power and enter data manually
        </span>
      </div>
      <div class="p30"></div>
      <div class="col-sm-12 col-md-12 col-lg-12 col-12">
        <div class="row ">
          <button class="le-button le-button1 le-margin">
            <img class="le-buuton-icon" src="~assets/sun.svg" />Solar
          </button>
          <button class="le-button le-button1 le-margin">
            <img class="le-buuton-icon" src="~assets/barrel.svg" />Diesel
          </button>
          <button class="le-button le-button1 le-margin">
            <img class="le-buuton-icon" src="~assets/wind1.svg" />Wind
          </button>
          <button class="le-button le-button1">
            <img class="le-buuton-icon" src="~assets/leaves.svg" />Compost
          </button>
        </div>
        <div class="p5"></div>
      </div>
...
export default {
  ...
  methods: {
    loadDataMeter(meter) {
      ...
      this.$http
        ...
        .then(function(response) {
          ...
          self.loadedmetername = meter.name
        })
    ...
    loadLeedConfiguration() {
      ...
      this.$http
        .get('/leed/energy/' + openLeedId + '?meterType=electricity')
        .then(function(response) {
          let resp = response.data
          console.log(resp)
