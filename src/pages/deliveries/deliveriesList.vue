  ...
  >
    <template #header>
      ...
      <template v-if="!showSearch">
        <button
          ...
          @click="addRecord()"
        >
          {{ $t('deliveries.deliveries.new_delivery') }}
        </button>
      </template>
    </template>
    ...
    <template #content>
        ...
        >
          ...
          >
            ...
            <div class="q-item-label nowo-label">
              {{ $t('deliveries.deliveries.no_data_deliveries') }}
            </div>
          </div>
          <template v-else>
            ...
            >
              ...
              >
                <template v-slot="scope">
                  <div v-if="isSignatureField(field)">
                    <SignatureField
                      :field="(field || {}).field"
                      :record="scope.row"
                    />
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/dl/deliveries/viewmanager`"
...
import { isDecimalField, isFileTypeField } from '@facilio/utils/field'
import {
  isWebTabsEnabled,
  findRouteForModule,
  pageTypes,
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Deliveries'
    },
    appliedFilters() {
      let {
        $route: { query },
  ...
  methods: {
    async loadRecords(force = null) {
      ...
      if (error) {
        let { message } = error
        this.$message.error(
          message || 'Error Occured while fetching deliveries list'
        )
      } else {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`deliveries.deliveries.delete_delivery`),
        message: this.$t(`deliveries.deliveries.delete_confirmation_delivery`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          let { message } = error
          this.$message.error(
            message || 'Error Occured while deleting delivery'
          )
          this.loading = false
        } else {
          this.$message.success(
            this.$t(`deliveries.deliveries.delete_success_delivery`)
          )
    ...
    checkDecimalType(fieldObj) {
      let { field } = fieldObj || {}
      return isDecimalField(field)
    },
    isSignatureField(field) {
      return this.$getProperty(field, 'field.displayType') === 'SIGNATURE'
    },
    isFileField(field) {
      let fieldObj = this.$getProperty(field, 'field')
      return isFileTypeField(fieldObj)
    },
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-delivery' })
      }
    },
    editRecord({ id = null } = {}) {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'edit-delivery', params: { id } })
      }
    },
