  ...
  methods: {
    ...
    switchView(view) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = `/app/dl/deliveries/${view.name}/${null}/overview`
    ...
    async loadData(loadMore, forceFetch) {
      ...
      if (error) {
        let { message = 'Error Occured while fetching Delivery list' } = error
        ...
        this.canLoadMoreList = listCount > (this.deliveryList || []).length
      }
      this.loading = false
