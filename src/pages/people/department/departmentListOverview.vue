  ...
  methods: {
    ...
    switchView(view) {
      ...
      if (isWebTabsEnabled()) {
        ...
        let url = `/app/pl/department/${view.name}/${null}/overview`
    ...
    async loadData(loadMore, forceFetch) {
      ...
      if (error) {
        ...
        if (forceFetch) {
          ...
          if (loadMore) {
            this.departmentList = [...(this.departmentList || []), ...list]
          }
        }
        let listCount = this.$getProperty(meta, 'pagination.totalCount', null)
        this.canLoadMoreList = listCount > (this.departmentList || []).length
      }
      this.loading = false
