  ...
  >
    <template #header>
      ...
      <template v-if="!showSearch">
        <button
          ...
          @click="addRecord()"
        >
          {{ $t('people.department.new_department') }}
        </button>
      </template>
    </template>
    ...
    <template #content>
        ...
        >
          ...
          >
            ...
            <div class="q-item-label nowo-label">
              {{ $t('people.department.no_data_department') }}
            </div>
          </div>
          <template v-else>
      ...
      <portal to="view-manager-link">
        <router-link
          tag="div"
          :to="`/app/pl/department/viewmanager`"
...
export default {
  ...
  computed: {
    ...
    moduleDisplayName() {
      return this.metaInfo?.displayName || 'Department'
    },
    appliedFilters() {
      let {
        $route: { query },
  ...
  methods: {
    ...
    async deleteRecords(idList) {
      let value = await this.$dialog.confirm({
        title: this.$t(`people.department.delete_department`),
        message: this.$t(`people.department.delete_confirmation_department`),
        rbDanger: true,
      ...
      if (value) {
        ...
        if (error) {
          ...
          this.$message.success(
            this.$t(`people.department.delete_success_department`)
          )
    ...
    addRecord() {
      if (isWebTabsEnabled()) {
        ...
        this.$router.push({ name: 'new-department' })
      }
    },
    editRecord({ id = null } = {}) {
      if (isWebTabsEnabled()) {
