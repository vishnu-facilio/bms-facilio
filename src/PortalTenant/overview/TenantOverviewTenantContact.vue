<template>
  <div class="p20 pT0 fc-tenant-contact-page">
    <div>
...
export default {
  ...
  methods: {
    async loadRecords() {
      ...
      let { list, meta, error } = await API.fetchAll('tenantcontact', params)
      if (error) {
        let { message } = error
        this.$message.error(
    ...
    goToList() {
      let route = findRouteForModule('tenantcontact', pageTypes.LIST)
