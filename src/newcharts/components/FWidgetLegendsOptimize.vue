<template>
  <div
    ...
    v-if="(options && options.widgetLegend.show) || isTreeMap"
  ...
  >
    <template v-if="isPrinting">
      <div v-for="(legend, idx) in legends" :key="idx" class="variance-item">
        ...
        <div
          :class="{ 'variance-data': legend.showColor && !isBaseLineSelected }"
        >
          <div
    ...
    <el-carousel height="100px" :autoplay="false" v-else>
      ...
      >
        <template v-for="(legend, idx) in item">
          <div
...
export default {
  ...
  computed: {
    ...
    isTreeMap() {
      if (this.options && this.options.type === 'treemap') {
        return true
      }
      return false
    },
  ...
  methods: {
    value(chart, data, options, reportVarianceData) {
      this.chart = chart
      this.data = data
      this.options = options
      this.reportVarianceData = reportVarianceData
      this.initData()
    },
    initData() {
      this.legends = []
      this.item = []
      if (
        (this.options && !this.options.widgetLegend) ||
        !this.reportVarianceData
      ) {
        return
      }

      this.allPoints.forEach(dp => {
        this.options.widgetLegend.variances[dp.alias].forEach(variance => {
    ...
    setVariance(dp, variance) {
      ...
      let varianceVal = this.reportVarianceData[dp.alias + '.' + variance]
      ...
      let legend = {
        ...
        showColor:
          !this.isTreeMap &&
          (this.isBaseLineSelected || this.allPoints.length > 1) &&
          dp.visible,
        dataPoint: dp,
      }
      ...
      if (!this.isTreeMap && this.data.x) {
        if (this.xLabelVariance.includes(variance) && this.data[dp.alias]) {
          let xAxisDataType = this.options.axis.x.datatype
          this.data[dp.alias].forEach((val, idx) => {
