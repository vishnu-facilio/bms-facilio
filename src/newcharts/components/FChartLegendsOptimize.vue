<template>
  ...
  >
    <template v-for="legend in legends">
      <div
        v-if="legend.type === 'datapoint' || legend.type === 'rangeGroup'"
        v-show="getVisibledDataPoints(legend)"
        :key="legend.key"
      ...
      >
        <div
          class="mL10 mR5"
          v-if="isScatterPlot && legend.pointType != 3"
      ...
      >
        <div
          class="mL10 mR5"
          v-if="isScatterPlot && child.pointType !== 3"
    ...
    <el-dialog
      ...
      :lock-scroll="true"
    >
      <div v-if="groupedByTime" class="overflow-y-scroll height250 pB70">
        <el-row
          class="pB5 border-bottom4 pT5"
...
// import shapes from '../../pages/assets/graphics/shapes'

export default {
  mixins: [AnalyticsMixin, NewDataFormatHelper],
  props: ['config', 'chart', 'resultObj'],
  data() {
    return {
      options: null,
      multichart: null,
  ...
  computed: {
    groupedByTime() {
      if (this.options && this.options.isGroupedByTime) {
        return true
      }
      return false
    },
    isScatterPlot() {
      if (this.config && typeof this.config.scatterConfig !== 'undefined') {
        return true
      }
      return false
    },
    ...
    scatterLegend() {
      return this.legends.find(dp => dp.key === this.selectedScatterDataPoint)
    },
    allPoints() {
      let points = []
  ...
  methods: {
    values(options, xValueMode, xLabelMap) {
      this.options = options
      this.xValueMode = xValueMode
