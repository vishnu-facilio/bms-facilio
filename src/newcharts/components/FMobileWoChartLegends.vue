<template>
  <div class="mobile-chart-legend">
    ...
    >
      ...
      <div v-if="!time" class="empty-time"></div>
    </div>
    <div
      v-else-if="
        legends &&
    ...
    <div
      v-if="xDataMap && xValueMode"
      class="fLegendContainer fLegendContainer-new fLegendContainer-right mobile-legend single-leagend active"
    >
      <template v-for="(legend, key) in xDataMap">
        <div
          :key="key"
          class="legendBoxNew legendBoxNew-fchart f-legendnew text-left active"
          :title="legend.label"
        >
          <el-color-picker
            style="padding: 10px;"
            v-model="xDataMap[key]"
            size="mini"
          ...
          <div
            ...
            class="textoverflow-ellipsis text-left newwo-leagend width100"
          >
            <div class="mobile-label" style="width:50%;">{{ key }}</div>
            <div
              class="mobile-tootltip-value f20"
              v-if="toolTipEnumValue && toolTipEnumValue.id === key"
            >
              {{ toolTipEnumValue.value }}
            </div>
          </div>
          <span
            class="drill-down-arrow"
            @click="handleDrillDown(key)"
            v-if="toolTipEnumValue && toolTipEnumValue.id === key && dataPoints"
          >
      ...
      <template v-for="(legend, index) in legends">
        ...
        >
          ...
          >
            ...
            <div class="mobile-tootltip-value f20">
              {{
          ...
          <span
            class="drill-down-arrow"
            @click="handleDrillDown(legend.key)"
            v-if="dataPoints"
          >
            <i class="el-icon-right"></i>
          </span>
        ...
        >
          ...
          >
            ...
            <span class="mobile-tootltip-value f20">
              {{
          ...
          <span
            class="drill-down-arrow"
            @click="handleDrillDown(child.key)"
            v-if="dataPoints"
          >
            <i class="el-icon-right"></i>
          </span>
...
export default {
  ...
  data() {
    return {
      ...
      toolTipEnumName: null,
      toolTipEnumValue: null,
      aggregateFunctions: [
        {
          label: 'TOTAL',
          value: 3,
  ...
  methods: {
    handleDrillDown(key) {
      let d = this.dataPoints.find(rt => rt.id === key)
    ...
    showTooltip(d, config, title) {
      ...
      let tMapUnit = {}
      let time = null
      if (this.xDataMap && this.xValueMode) {
        this.toolTipEnumValue = d[0] || null
      } else {
        for (let i = 0; i < d.length; i++) {
          tMap[d[i].id] = d[i].value !== null ? d[i].value : 'No data'
          tMapUnit[d[i].id] =
            config.dataPoint.find(rt => rt.key === d[i].id) &&
            config.dataPoint.find(rt => rt.key === d[i].id).unitStr
              ? d[i].value !== null
                ? config.dataPoint.find(rt => rt.key === d[i].id).unitStr
                : ''
              : ''
          if (
            config.dataPoint &&
            config.dataPoint.length &&
            config.dataPoint[0].type &&
            config.dataPoint[0].type === 'group' &&
            config.dataPoint[0].children &&
            config.dataPoint[0].children.length
          ) {
            tMapUnit = {}
            tMap = {}
            tdate[d[i].id] =
              config.dataPoint.find(function(row) {
                if (row.children) {
                  let data = row.children.find(rt => rt.key === d[i].id)
                  if (data) {
                    return data
                  }
                }
            ...
            if (config.data && config.data.length) {
              config.data.find(function(row) {
                if (row.children) {
                  row.children.find(function(rt) {
                    tMap[rt.key] =
                      rt.rawValue !== null ? rt.rawValue : 'No data'
                    tMapUnit[rt.key] =
                      rt.rawunit && rt.rawValue !== null ? rt.rawunit : ''
                  })
                }
              })
            }
          ...
          } else if (config.dataPoint && config.dataPoint.length) {
            if (
              typeof config.dataPoint.find(rt => rt.key === d[i].id) !==
                'undefined' &&
              config.dataPoint.find(rt => rt.key === d[i].id).aggr
            ) {
              tdate[d[i].id] =
                config.dataPoint.find(rt => rt.key === d[i].id).aggr === 3
                  ? 'TOTAL'
                  : 'AVG'
              time = title || moment(d[i].x).format('DD MMM hh:MM a')
              tMap2[d[i].id] = title || moment(d[i].x).format('DD MMM hh:MM a')
            } else {
              tdate[d[i].id] = d[i].value
              tMap2[d[i].id] = d[i].name
            }
          } else {
