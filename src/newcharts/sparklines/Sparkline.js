...
export default {
  ...
  methods: {
    ...
    updateData() {
      ...
      if (rect) {
        tooltip.style.left = `${this.curEvt.cx + rect.width * 0.25}px`
        tooltip.style.top = `${this.curEvt.cy - rect.height}px`
        try {
          tooltip.innerHTML =
            this.tooltipProps.formatter(curData) || tooltipContent
        } catch (e) {
