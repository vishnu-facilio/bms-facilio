<template>
  <el-dialog
    custom-class="wo-kfi-target f-kfi-card-builder fc-dialog-center-container"
    :append-to-body="true"
  ...
  >
    <span slot="title" class="dialog-footer">
      <span class="kpi-card-header">TARGET METER</span>
      <el-select
      ...
      >
        <el-option
          v-for="(m, mIdx) in getLicenseEnabledModules"
          :key="mIdx"
          :value="m.moduleName"
          :label="m.label"
        ></el-option>
    ...
    <div style="height:500px;">
      ...
      <template v-for="(expression, index) in data.expressions" v-else>
        ...
        >
          ...
          <el-form-item prop="category">
            <el-col :span="19">
              <p class="grey-text2 kpi-text-3 ">Target Metric</p>
              <el-select
                style="width: 100%"
          ...
          <el-form-item prop="category">
            <el-col :span="19">
              <p class="grey-text2 kpi-text-3 ">Target Constant</p>
              <el-input
                style="width: 100%"
                :autofocus="true"
                class="addReading-title el-input-textbox-full-border"
                v-model="data.targetConstant"
                placeholder="Target Constant"
              ></el-input>
            </el-col>
          </el-form-item>
            ...
            <el-col :span="12">
              ...
              >
                <el-option
                  ...
                  v-for="(time, index) in data.timeFields"
                  :key="index"
                ></el-option>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      if (this.cardData) {
        this.data = this.deforamtData(this.cardData)
    ...
    validateData() {
      if (this.data && this.data.expressions.length) {
        if (this.data.expressions[0].actualName === null) {
          ...
          let { conditions } = this.data.expressions[0].criteria
          let criteriaObjValues = Object.values(conditions)

          let hasError = criteriaObjValues.every(field => {
            if (!isEmpty(field)) {
    ...
    prepareMetrics(result) {
      ...
      if (result && result.meta && result.meta.metrics) {
        ...
        let defaultMetric = {}
        defaultMetric['displayName'] = 'Number of ' + this.data.moduleName + 's'
