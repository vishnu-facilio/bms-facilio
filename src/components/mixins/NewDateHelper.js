...
export default {
  ...
  getDatePickerObject(id, value, timeZone) {
    ...
    for (let key in Object.keys(dateOperators)) {
      ...
      for (let operationsIndex in operationsList) {
        let temp = operationsList[operationsIndex]
        if (temp.operatorId === id) {
          dateObj['operatorId'] = id
          dateObj['operationOnId'] = temp.operationOnId
          dateObj['operationOn'] = temp.operationOn
          dateObj['label'] = temp.label
          if (Array.isArray(value)) {
            ...
            let offset = -toMoment.diff(fromMoment, temp.operationOn) - 1
