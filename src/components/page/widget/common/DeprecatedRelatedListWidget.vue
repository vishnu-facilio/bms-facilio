...
import ListPopup from 'pageWidgets/common/line-items/RelatedListPopup'
...
export default {
  ...
  computed: {
    ...
    canShowDelete() {
      let { $account, moduleName, hasDeleteAccess } = this
      let hasModulePermission = ['custom_purchaseorderlineitems'].includes(
        moduleName
      )
      //this is specially handled for investa organization
      return $account.org.orgId === 17 && hasModulePermission && hasDeleteAccess
    },
    hasDeleteAccess() {
      let {
        moduleName,
    ...
    canShowQuickCreate() {
      ...
      return (
        ['vendorDocuments', 'clientcontact', 'custom_tenantutility'].includes(
          moduleName
        ) || showQuickCreate
      )
    },
    ...
    currentModuleDetails() {
      ...
      return {
        ...
        lookupModuleDisplayName: relatedRecordDisplayName,
