    ...
    <div class="related-list-header">
      <div class="header justify-content-space">
        <div class="flex-middle">
          <div
            class="releated-widget-title d-flex flex-direction-column justify-center mL0"
          >
            {{ $t('common.products.time_log') }}
          </div>
        </div>
        <div class="flex-middle">
          <!-- <div class="pT20">
            <el-checkbox v-model="timerEnabled" @change="reloadLog">{{
              $t('setup.setupLabel.timer_enabled')
            }}</el-checkbox>
          </div> -->
          <!-- <div class="pR10">
            <div class="fc-black-13 text-left bold pB5 pL10">
              {{ $t('setup.setup_profile.state') }}
            </div>
            <el-select
              v-model="statusModelData"
              placeholder="select"
              @change="reloadLog"
            ...
            >
              <el-option
                v-for="(status, inex) in stateListData"
                :key="inex"
...
export default {
  props: ['details', 'moduleName', 'resizeWidget'],
