<template>
  <div class="p30 pT40 rule-associated-wo-card">
    ...
    <div v-if="values">
      ...
      <div class="flex-middle pT10">
        <div class="text-left width50 border-right3">
          <div class="fc-black-com f24">
            {{ values.currentWeekWo ? values.currentWeekWo : 0 }}
          </div>
          <div class="label-txt-blue f12 bold pT10">
            {{ $t('common.events.this_week') }}
          </div>
        </div>
        <div class="text-left width50 pL30">
          <div class="fc-black-com f24">
            {{ values.currentMonthWo ? values.currentMonthWo : 0 }}
          </div>
  ...
  computed: {
    ruleId() {
      ...
      if (moduleName === 'newreadingrules') {
        ...
        ruleId = this.$getProperty(details, 'preRequsite.id', null)
      }
      return ruleId
    },
  },
...
<style scoped>
.rule-associated-wo-card span.period {
  font-size: 14px;
